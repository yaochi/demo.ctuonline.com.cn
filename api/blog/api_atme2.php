...
if ($uid) {
	$value = DB :: fetch_first("select * from pre_common_user_at where uid=" . $uid);
	if ($value) {
		$ordersql = updownsql($pagetype, 'home_feed', 'feedid', $feedid, $shownum, 'feedid', "feedid in (" . $value[feedids] . ") and icon!='comment'  and icon!='profile'  and icon!='click' ");
		$feedquery = DB :: query("select * from " . DB :: TABLE("home_feed") . " where feedid in (" . $value[feedids] . ")" . $wheresql . " and icon!='comment'  and icon!='profile'  and icon!='click' " . $ordersql['ordersql']);
		while ($value = DB :: fetch($feedquery)) {
			if ($value[fid]) {
			...
			while ($commentvalue = DB :: fetch($commentquery)) {
				...
				$cvalue[commentContent] = cutstr($newcommentvalue[message],200);
