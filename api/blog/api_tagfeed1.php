...
if($tag){
	...
	$ordersql = updownsql($pagetype, 'home_feed', 'hf.feedid', $feedid, $shownum, 'feedid', implode(' AND ', $wheresql) . " and ( icon!='comment' and idtype!='gpicid' and idtype!='galbumid' and idtype!='picid' and icon!='profile' and icon!='sitefeed' and icon!='click' and idtype!='class' and idtype!='case' and idtype!='doc' and !(icon='share' and idtype=''))");
	$res['refresh'] = $ordersql['refresh'];
	$query = DB :: query("select * from ".DB::Table("home_tagrelation")." ht,".DB::TABLE("home_feed")." hf where ht.contentid=hf.feedid and ht.tagname='".$tag."' and ".implode(' AND ', $wheresql)." and ( hf.icon!='comment' and hf.idtype!='gpicid' and hf.idtype!='galbumid' and hf.idtype!='picid' and hf.icon!='profile' and hf.icon!='sitefeed' and hf.icon!='click' and hf.idtype!='class' and hf.idtype!='case' and hf.idtype!='doc' and !(hf.icon='share' and hf.idtype=''))" . $ordersql['ordersql']);
