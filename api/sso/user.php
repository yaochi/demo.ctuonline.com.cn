...
if($method=="post"){
    ...
    if($v=="1"){
        ...
        if($code!=$securecode){
            ...
            switch($method){
            	case 'create':
            		$result = $user->create($uid, $userinfo);
            		break;
            	case 'update':
            		$result = $user->update($uid, $userinfo);
            		break;
            	case 'delete':
            		$result = $user->remove($uid);
            		break;
            	default:
...
class User{
	function create($uid, $userinfo) {
		...
		$newuid = uc_user_register_uid($uid, $userinfo['username'], $userinfo['password'], empty($userinfo['email'])? "" : $userinfo['email']);
		...
		if(!empty($userinfo['realname'])){
			...
			//$profile['nickname'] = $userinfo['realname'].'_'.$newuid;
		}
	...
	function update($uid, $userinfo) {
		...
		if(!empty($userinfo['realname'])){
			$profile['realname'] = $userinfo['realname'];
			//$profile['nickname'] = $userinfo['realname'].'_'.$uid;
		}
		$profile['gender'] = ($userinfo['gender'] == 1 ? 1 : 2);
		if(!empty($userinfo['birthday'])){
			$birthday = explode("/", $userinfo['birthday']);
