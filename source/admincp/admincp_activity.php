...
<?
	...
	if(!submitcheck('submit', 1)) {
		...
		list($page, $start_limit, $groupnum, $conditions, $urladd) = countactivitys(); 
		$multipage = multi($groupnum,20, $page, ADMINSCRIPT."?action=activity&operation=manage&submit=yes".$urladd);
		$query = DB::query("SELECT f.fid, f.fup, f.type, f.name, f.posts, f.threads, ff.membernum, ff.lastupdate, ff.dateline, ff.foundername, ff.founderuid FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON f.fid=ff.fid
			WHERE status='3' AND type='activity' AND $conditions LIMIT $start_limit, ".$_G['setting']['group_perpage']);
		...
		while($group = DB::fetch($query)) {
			$groups .= showtablerow('', array('class="td25"', '', ''), array(
				...
				"<a href=\"".ADMINSCRIPT."?action=activity&operation=editgroup&fid=$group[fid]\" class=\"act\">".cplang('detail')."</a>"
			), TRUE);
		}

		shownav('group', '活动管理');
		showsubmenu('活动管理');
		showformheader("activity&operation=deletegroup");
		showtableheader(cplang('activity_search_result', array('groupnum' => $groupnum)).' <a href="javascript:history.go(-1);" class="act lightlink normal">'.cplang('research').'</a>');
	...
	$group = DB::fetch_first("SELECT f.fid, f.fup, f.name, f.status, f.displayorder, f.org_id, ff.* FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON f.fid=ff.fid WHERE status='3' AND type='activity' AND $condition");
	...
	if(!submitcheck('editsubmit')) {
		...
		showformheader("activity&operation=editgroup&fid=$fid", 'enctype');
		showtableheader();
		showsetting('活动名', 'namenew', $group['name'], 'text');
		...
		showsetting('活动图片', 'iconnew', '', 'file', '', 0, $groupicon);
		showsubmit('editsubmit');
		showtablefooter();
		showformfooter();
...
/*            "stattion"=>$station_input_id,
		...
		cpmsg('活动编辑成功', 'action=activity&operation=editgroup&fid='.$fid, 'succeed');
	}
...
function searchgroups($submit) {
	...
	showformheader("activity&operation=manage");
	showtableheader();
	showsetting('活动名称', 'srchname', $srchname, 'text');
	showsetting('活动ID', 'srchfid', $srchfid, 'text');
	//showsetting('活动分类', '', '', '<select name="selectgroupid[]" multiple="multiple" size="10"><option value="all"'.(in_array('all', $_G['gp_selectgroupid']) ? ' selected' : '').'>'.cplang('unlimited').'</option>'.$groupselect.'</select>');
