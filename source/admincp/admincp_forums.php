 ...
 *
 *      $Id: admincp_forums.php 11774 2010-06-13 00:44:17Z monkey $
 */
	...
	if(!submitcheck('detailsubmit')) {
		...
		foreach($_G['cache']['forums'] as $forums) {
			if($forums['type'] == 'group') {
				...
				$forumselect .= '<input class="left checkbox ck" chkvalue="'.$sgid.'" name="multi[]" value="'.$forums['fid'].'" type="checkbox" /><a class="f'.($fid == $forums['fid'] || in_array($forums['fid'], $_G['gp_multi']) ? ' current"' : '').'" href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=forums&operation=edit&switch=yes&fid='.$forums['fid'].($forum['type'] != 'group' ? '&anchor=\'+currentAnchor' : '\'').'+\'&scrolltop=\'+scrollTopBody()">'.$forums['name'].'</a>';
			} elseif($forums['type'] == 'sub') {
				$forumselect .= '<input class="left checkbox ck" chkvalue="'.$sgid.'" name="multi[]" value="'.$forums['fid'].'" type="checkbox" /><a class="s'.($fid == $forums['fid'] || in_array($forums['fid'], $_G['gp_multi']) ? ' current"' : '').'" href="###" onclick="location.href=\''.ADMINSCRIPT.'?action=forums&operation=edit&switch=yes&fid='.$forums['fid'].($forum['type'] != 'group' ? '&anchor=\'+currentAnchor' : '\'').'+\'&scrolltop=\'+scrollTopBody()">'.$forums['name'].'</a>';
			}
		}
