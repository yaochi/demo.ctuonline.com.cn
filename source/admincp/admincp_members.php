 ...
 *
 *      $Id: admincp_members.php 11738 2010-06-12 01:38:11Z monkey $
 */
...
if($operation == 'search') {
	...
	if(empty($_G['gp_uid']) && empty($_G['gp_username']) && empty($_G['gp_ip'])) {
		...
		if($membernum) {
			...
			$query = DB::query("SELECT m.uid AS uid, m.username AS username, m.adminid AS adminid, m.groupid AS groupid, m.credits AS credits,
				...
				LEFT JOIN ".DB::table('common_member_status')." ms ON m.uid=ms.uid WHERE 1 $repeatip");
		...
		echo $members;
		showtablerow('', array('class="td25"', 'class="lineheight" colspan="7"'), array('', cplang('members_admin_comment')));
		showsubmit('submit', 'submit', '<input type="checkbox" name="chkall" onclick="checkAll(\'prefix\', this.form, \'uidarray\')" class="checkbox">'.cplang('del'));
		showtablefooter();
		showformfooter();
	...
	if(!submitcheck('submit', 1)) {
		...
		$uids = 0;
		$extra = '';
       	if(empty($search_condition[userarr])){
       		$uids=array(-1);
       	} else{
...
EOT;
	...
	foreach($_G['cache']['profilesetting'] as $fieldid=>$field) {
		if($field['available']) {
			$_G['cache']['profilesetting'][$fieldid]['unchangeable'] = 0; // HACK，避免设为不可修改的项，管理员也无法修改
			...
			if($_POST['do'] == 'pass') {
				...
				while($value = DB::fetch($query)) {
					...
					if($plus_group && empty($updated_members[$value['uid']])) {
						...
						if($plus_group['type'] == 'member' && $member['credits'] < $plus_group['creditshigher']) {
							DB::update('common_member', array('credits'=>$plus_group['creditshigher'], 'groupid'=>$plus_group['groupid']), array('uid'=>$value['uid']));
						} elseif($plus_group['type'] == 'special') {
							DB::update('common_member', array('groupid'=>$plus_group['groupid']), array('uid'=>$value['uid']));
						}
...
SEARCHFORM;
	...
	$query = DB::query('SELECT groupid, grouptitle, `type` FROM '.DB::table('common_usergroup')." WHERE type IN ('member', 'special')");
