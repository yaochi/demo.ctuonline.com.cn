 ...
 *
 *      $Id: admincp_plugins.php 11456 2010-06-03 04:52:08Z monkey $
 */
...
if(!$operation) {
	...
	memory("rm", "group_load_plugin");
	updatecache(array('plugin', 'setting', 'styles', 'blockclass'));
	updatemenu();
	cpmsg('plugins_edit_succeed', 'action=plugins', 'succeed');
	...
	if($finish) {
		memory("rm", "group_load_plugin");
		updatecache('setting', 'blockclass');
		updatemenu();
		if($operation == 'plugininstall') {
			pluginstat('install', $pluginarray['plugin']);
	...
	if(!submitcheck('editsubmit')) {
        ...
        $varname = array("adminid", array());
        foreach($_G["common"]["group_type"] as $value){
            $varname[1][] = array($value[0], $value[1], '1');
        }
        $ids = $plugin['modules']['extra']["adminid"];
        showtitle('可以使用该组件的专区');
        showsetting('', $varname, $ids , 'omcheckbox');
		showsubmit('editsubmit');
		showtablefooter();
		showformfooter();
		showtagfooter('div');
		...
		if(is_array($plugin['modules'])) {
			foreach($plugin['modules'] as $moduleid => $module) {
				...
				$typeselect = '<optgroup label="'.cplang('plugins_edit_modules_type_g1').'">'.
					...
					'</optgroup>'.get_join_add_plugin_point_option($module);
                */
                $typeselect = get_join_add_plugin_point_option($module);
		...
		showtablerow('', array('class="td25"', 'class="td28"'), array(
			...
			'<select id="s_n" onchange="shide(this, \'n\')" name="newtype">'.
				...
				'</optgroup>'.*/get_join_add_plugin_point_option().
			'</select>',
			...
			'<option value="3">'.cplang('usergroups_system_3').'</option>'.*/get_join_add_plugin_adminid_option().
			'</select></span>',
		...
		if($type == 'common') {
            ...
            $plugin["modules"]["extra"]["adminid"] = $_G["gp_adminid"];
				...
				foreach($plugin['modules'] as $moduleid => $module) {
					if(!isset($_G['gp_delete'][$moduleid])) {
						...
						$modulesnew[] = array(
							...
							'adminid'	=> $_G['gp_adminidnew'][$moduleid],
