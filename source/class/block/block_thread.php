 ...
 *
 *      $Id: block_thread.php 11781 2010-06-13 02:14:04Z xupeng $
 */
...
class block_thread {
	 ...
	 * 
	 */
	function getthreadNew($tid, $messagelength = 80, $poll ) {
		$nospecial=false;
		$thread=$poll;
		global $_G;
		if(!$tid) {
...
//		print_r("thread['special']".$thread['special'] );
		if($thread['special'] == 1) {
			//$multiple = DB::result_first("SELECT multiple FROM ".DB::table('forum_poll')." WHERE tid='$tid'");
			$multiple=$poll['multiple'];
			if($poll['allvote']){
				$allvote=$poll['allvote'];
			}else{
				$allvote=check_user_group($_G['username'],$_G['fid']);;
			}
			$optiontype = $multiple ? 'checkbox' : 'radio';
			$query = DB::query("SELECT polloptionid, polloption FROM ".DB::table('forum_polloption')." WHERE tid='$tid' ORDER BY displayorder");
			while($polloption = DB::fetch($query)) {
