 ...
 *
 *      $Id: class_core.php 11786 2010-06-13 02:29:06Z zhengqingpeng $
 */
...
function shutdown(){
	$info = "<!--";
	foreach(DB::object()->sqldebug as $item){
		$info = $info . $item[0]."\n\r";
	}
	$info = $info . "--!>";
	echo $info;
...
class discuz_core {
	...
	function discuz_core() {
            //add by joinsoft
            require_once(dirname(dirname(__FILE__))."/joinplugin/common.inc.php");
	...
	function _init_env() {
		...
		$_G['siteurl'] = htmlspecialchars('http://'.$_SERVER['HTTP_HOST'].preg_replace("/\/+(api)?\/*$/i", '', substr($_G['PHP_SELF'], 0, strrpos($_G['PHP_SELF'], '/'))).'/');//涉及端口问题
		$_G['siteroot'] = substr($_G['PHP_SELF'], 0, -strlen(basename($_G['PHP_SELF'])));
        
        $_G["common"]["group_type"] = array(1=>array(1, "机构专区", template=>"_official"),
                                            20=>array(20, "业务专区"),
                                            60=>array(60, "兴趣专区", template=>"_interest"),
                                            80=>array(80, "临时专区", template=>"_temp"),
                                                    );
        /*foreach ($_G["common"]["group_type"] as $key => $value) {
            if ($value["subs"]) {
                foreach($value["subs"] as $k=>$v){
                    $_G["common"]["group_type_template"][$k] = $v;
                }
            }else{
                $_G["common"]["group_type_template"][$key] = $value;
            }
        }*/
	...
	function _init_config() {
		...
		if(empty($this->config['debug']) || !file_exists(libfile('function/debug'))) {
			...
			if($this->config['debug'] == 2 || $_GET["debug"]=="true") {
				error_reporting(E_ALL ^ E_NOTICE ^ E_WARNING);
			}
			if($this->config['sql_debug'] || $_GET["debug"]=="true"){
	...
	function _init_output() {
		...
		if($this->config['output']['gzip'] && empty($this->var['gp_inajax']) && $this->var['gp_mod'] != 'attachment' && EXT_OBGZIP) {
	...
	function _get_true_ip() {
		$clientip = '';
		if(getenv('HTTP_CLIENT_IP') && strcasecmp(getenv('HTTP_CLIENT_IP'), 'unknown')) {
			$clientip = getenv('HTTP_CLIENT_IP');
		} elseif(getenv('HTTP_X_FORWARDED_FOR') && strcasecmp(getenv('HTTP_X_FORWARDED_FOR'), 'unknown')) {
			$clientip = getenv('HTTP_X_FORWARDED_FOR');
		} elseif(getenv('HTTP_TRUEIP') && strcasecmp(getenv('HTTP_TRUEIP'), 'unknown')) {
			$clientip = getenv('HTTP_TRUEIP');
		} elseif(isset($_SERVER['HTTP_TRUEIP']) && $_SERVER['HTTP_TRUEIP'] && strcasecmp($_SERVER['HTTP_TRUEIP'], 'unknown')) {
			$clientip = $_SERVER['HTTP_TRUEIP'];
		}elseif(isset($_SERVER['REMOTE_ADDR']) && $_SERVER['REMOTE_ADDR'] && strcasecmp($_SERVER['REMOTE_ADDR'], 'unknown')) {
			$clientip = $_SERVER['REMOTE_ADDR'];
		}
	...
	function _init_db1() {
		$this->db = & DB::object();
		$this->db->set_config($this->config['db2']);
		$this->db->connect();
	}
	//end
	...
	function _init_misc() {
		...
		if($this->init_user) {
			if($this->var['group'] && isset($this->var['group']['allowvisit']) && !$this->var['group']['allowvisit']) {
				...
				} elseif(CURSCRIPT == 'home' && $this->var['gp_mod'] == 'misc' && $this->var['gp_mod'] == 'misc' && $this->var['gp_ac'] == 'swfupload') {
				} elseif(!in_array(CURSCRIPT, array('member', 'misc', 'api')) && !$this->var['inajax']) {
		...
		if($this->var['setting']['bbclosed'] && !(in_array(CURSCRIPT, array('admin', 'member', 'misc', 'api')) || getglobal('adminid', 'member') == 1 || $this->var['inajax'])) {
			...
			showmessage_template($closedreason ? $closedreason : 'board_closed', "#", array(), array(), 0, "common/showmessage_close");
		}
...
class discuz_memory
{
	...
	function init($config) {
		...
		if($config['redis']['group']){
			if($this->extension['redis'] && $config['redis']['on'] && !empty($config['redis']['server'])) {
				require_once libfile('class/redis');
				$this->memory = new discuz_redis();
		...
		if(!is_object($this->memory) && $this->extension['memcache'] && !empty($config['memcache']['server'])) {
