 ...
 *
 *      $Id: class_upload.php 11349 2010-06-01 00:57:49Z cnteacher $
 */
...
Class discuz_upload{
	...
	function save() {
		...
		if(empty($this->attach) || empty($this->attach['tmp_name']) || empty($this->attach['target'])) {
			...
			$this->errorcode = -102;
		} elseif(in_array($this->type, array('common')) && (!$this->attach['isimage'] && $this->attach['ext'] != 'ext' && $this->attach['ext'] != 'swf')) {
			$this->errorcode = -102;
	...
	function get_image_info($target, $allowswf = false) {
		...
		if(!$isimage && ($ext != 'swf' || !$allowswf)) {
			...
			if($size > 16777216 || $size < 16 ) {
                        ...
                        elseif($ext == 'swf' && !in_array($type, array(4,13))) {
                            return false;
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		if($type == 'album' || $type == 'forum' || $type == 'portal' || $type == 'category' || $type == 'home' || !(strpos($type, 'plugin_') === false)) {
			$subdir1 = date('Ym');
			$subdir2 = date('d');
			$subdir = $subdir1.'/'.$subdir2.'/';
	...
	function check_dir_type($type) {
		if(in_array($type, array('forum', 'group', 'album', 'portal', 'common', 'temp', 'category','home'))){
			return $type;
		}else if(!(strpos($type, 'plugin_') === false)){
			return $type;
		}else{
			return 'temp';
		}
