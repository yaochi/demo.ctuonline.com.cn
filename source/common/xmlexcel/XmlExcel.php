...
{  
    ...
    {  
        $_temp="<Row ss:AutoFitHeight=\"0\">";  
            foreach($array as $key=>$val):  
                #读取指定数据类型,默认String  
                $_type=!emptyempty($this->_columnType)&&isset($this->_columnType)?!emptyempty($this->_columnType[$key])&&isset($this->_columnType)?$this->_columnType[$key]:"String":"String";  
    ...
    {  
        if($isErrorReport):  
            if(emptyempty($array)||!isset($array)||count($array)==0):  
    ...
    {  
        ...
        if($_hasData):  
            ...
            echo "<Worksheet ss:Name=\"{$this->convert($key)}\"><Table ss:DefaultColumnWidth=\"{$this->defaultWidth}\" ss:DefaultRowHeight=\"{$this->defaultHeight}\">";  
                ...
                #表格标题  
                if(!emptyempty($this->_title[$key]['title'])):  
    ...
    {  
        ...
        $is_convert=!emptyempty($convert_callback_function)&&function_exists($convert_callback_function);  
        $index=0;  
        $_ra=array();  
    ...
    {  
        if(!emptyempty($this->_column[$sheet]['style_width'])&&isset($this->_column[$sheet]['style_width'])):  
    ...
    {  
        ...
        if(emptyempty(self::$pre_workBook)):  
    ...
    {  
        return !emptyempty($str)&&is_string($str)?iconv($source_code,$target_code,$str):$str;  
    }  
    /** 
     * @desc 打印调试信息 
     * @param null 
