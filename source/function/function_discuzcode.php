...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '1', $pid = 0) {
	global $_G;
...
function parseflash($w, $h, $url) {
	...
	$url = $matches[0];
	return '<embed src='.$url.' type="application/x-shockwave-flash" width="'.$w.'" height="'.$h.'" quality="high" /><br />';
...
function parsemedia($params, $url) {
	...
	if(in_array(count($params), array(3, 4))) {
		...
		switch($type) {
			...
			case 'flv':
				return '<embed src='.rawurlencode($url).' type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'" quality="high" /><br />';
			...
			case 'swf':
				return '<embed src='.$url.' type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'" quality="high" /><br />';
...
function parseflv($url, $width = 0, $height = 0) {
	...
	if($flv) {
		if(!$width && !$height) {
			...
			return '<embed src='.$flv.' type="application/x-shockwave-flash" width="'.$width.'" height="'.$height.'" quality="high" /><br />';
