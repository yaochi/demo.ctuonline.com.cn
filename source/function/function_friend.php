...
function friend_all_list($uid) {
	...
	$query = DB::query("SELECT f.*,m.realname FROM ".DB::table('home_friend')." f left join ".DB::table('common_member_profile')." m on f.fuid=m.uid
		WHERE f.uid='$uid' ORDER BY f.num DESC, f.dateline DESC ");
	while ($value = DB::fetch($query)) {
		$list[$value['fuid']] = $value;
	}
	return $list;
...
function friend_make($touid, $tousername, $checkrequest=true) {
	...
	DB::query("REPLACE INTO ".DB::table('home_friend')." (uid,fuid,fusername,dateline,type)
		VALUES ('$touid', '$_G[uid]', '$_G[username]', '$_G[timestamp]','3'),
			('$_G[uid]', '$touid', '$tousername', '$_G[timestamp]','3')");
...
function friend_cache($touid) {
	...
	$count = 0;
	$fcount = 0;
	$query = DB::query("SELECT * FROM ".DB::table('home_friend')." WHERE uid='$touid' and type='3' ORDER BY num DESC, dateline DESC");
	while ($value = DB::fetch($query)) {
	...
	$query = DB::query("SELECT fuid,type FROM ".DB::table('home_friend')." WHERE uid='".$touid."' and (type='1' or type='3') ORDER BY num DESC, dateline DESC");
			$fcount=0;
			while ($value = DB::fetch($query)) {
				$fuids[]=$value['fuid'];
				if($value['type']=='3'){
	...
	DB::update('common_member_field_home', array('feedfollow'=>implode(',', $fuids)), array('uid'=>$touid));		
	DB::query("UPDATE ".DB::table('common_member_status')." SET follow=follow+1,fans=fans+1,friends='".$fcount."' WHERE uid='$touid'");
...
//alter by qiaoyz,2011-3-22,判断是否是好友
function isfriend($touid,$uid){
	global $_G;
	$result = DB::fetch_first("SELECT fuid FROM ".DB::table("home_friend")." WHERE uid=".$uid." AND fuid=".$touid);
    $fuid = $result?true:false;
}
?>