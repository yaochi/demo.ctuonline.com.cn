 ...
 *
 *      $Id: function_home.php 11397 2010-06-02 01:19:37Z zhengqingpeng $
 */
...
function getstr($string, $length, $in_slashes=0, $out_slashes=0, $censor=0, $bbcode=0, $html=0,$dot) {
	global $_G;
...
function ckprivacy($key, $privace_type) {
	...
	if($privace_type == 'feed') {
		...
		if(!$result && $privace_type == 'view' && $_G['adminid']==1) {
			$result = true;
		}
	}
	$_G[$var] = true;
...
function home_user_add_blog(){

	//修改 by songsp  2011-3-25 14:25:40
	//从cache中获得官方博客uid和姓名
    ...
    $query = DB::query("SELECT cm.uid ,cm.username,cmp.realname FROM ".DB::table('common_member')." cm LEFT JOIN ".DB::table("common_member_profile")." cmp ON cm.uid=cmp.uid WHERE cm.uid=".getOfficialBlogUid());
    $value=DB::fetch($query);
    if($value){
    	if($allowmem){
    		memory("set", $cache_key,serialize($value), 86400); //60*60*24  24小时
