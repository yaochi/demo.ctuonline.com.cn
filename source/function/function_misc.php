...
//		$thread['forumname'] = empty($_G['cache']['forums'][$thread['fid']]['name']) ? 'Forum' : $_G['cache']['forums'][$thread['fid']]['name'];
		$thread['forumname'] = get_groupname_by_fid($thread['fid']);
	}
...
function misc_get_forum_hot_config(){
    $config = array(threads=>0.4, viewnum=>0.3, replys=>0.2, members=>0.1);
    $value = DB::fetch(DB::query("SELECT value FROM ".DB::table("commmon_config")." WHERE `key`='threadhotprice'"));
    if($value){
        $config = unserialize($value["value"]);
    }
    return $config;
}


function misc_activity_num_str($num){
    $end = 5-floor($num);
    for($i=1; $i<=$end; $i++){
        $result .= '<li class="replaced grStr">'.$i.'</li>';
    }
    for($i=1; $i<=floor($num); $i++){
        $result .='<li class="replaced blStr">'.$i.'</li>';
    }
    return $result;
}

function misc_shlecturer_num_str($num){
    $end = 5-floor($num);
	$decimals=$num-floor($num);
	for($i=1; $i<=floor($num); $i++){
        $result .='<li class="replaced blStr" style="float:left;">'.$i.'</li>';
    }
	if($decimals){
		  $result .= '<li class="replaced hlStr" style="float:left;"></li>';
		  $end=$end-1;
	}
    for($i=1; $i<=$end; $i++){
		 $result .= '<li class="replaced grStr" style="float:left;">'.$i.'</li>';
    }
    return $result;
}
...
/**
 * type 可以是
 *  thread(话题) poll(投票) forum(专区) blog(日志) group(类别) album(个人相册) space(个人空间) galbum(专区相册) gpic(专区图片) nwkt(你我课堂)
 *  gnotice(通知公告) glive(直播) doc(文档) activity(活动)
 */
function misc_favorite_ranking_type($type, $size=10){
    switch($type) {
		case 'thread':
			$idtype = 'tid';
			break;
    ...
    $result = array();
    if($idtype){
        $sql = "SELECT * FROM ".DB::table("home_favorite")." WHERE hf.idtype='".$idtype."' GROUP BY id,idtype ORDER BY count(1) DESC LIMIT ".$size;
        $query = DB::query($sql);
...
function misc_esngrouporg($fid, $orgid){
	global $_G;
    misc_get_url($_G[config][misc][taskhost]."/esngrouporg?fid=".  urlencode($fid)."&orgid=".  urlencode($orgid));
}

function misc_enstation($fid, $sid){
	global $_G;
    misc_get_url($_G[config][misc][taskhost]."/esngroupstation?fid=".  urlencode($fid)."&sid=".  urlencode($sid));
}
?>