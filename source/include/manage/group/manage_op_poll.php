...
if($_G["gp_method"]=="search"){
  ...
  /*  require_once libfile("function/org");
    ...
    $sql = "SELECT #fileds# FROM ".DB::Table("forum_thread")." ft,".DB::Table("common_resource")." cr,"
        ...
        ." WHERE cm.uid=cmp.uid AND cm.uid=cr.uid AND ft.tid=cr.rid AND cr.rtype='poll'";
    ...
    if(!empty($_G[gp_content])){
        ...
        $sql = $sql." AND cr.rid IN(".implode(",", $pollids).")";
    }
    ...
    $url = "manage.php?mod=group&op=poll&method=search&code=true&usernames=$usernames&usernames_uids=$usernames_uids&content=$content&starttime=$starttime&endtime=$endtime";
  ...
  require_once libfile("function/credit");
   if(!empty($_G[gp_ridarray])){
	   foreach($_G[gp_ridarray] as $key => $rid){
	   		DB::query("DELETE FROM ".DB::table('forum_poll')." WHERE tid='$rid'");
			DB::query("DELETE FROM ".DB::table('forum_polloption')." WHERE tid='$rid'");
			DB::query("DELETE FROM ".DB::table('forum_thread')." WHERE tid='$rid'");
			DB::query("DELETE FROM ".DB::table('forum_threadmod')." WHERE tid='$rid'");
			credit_create_credit_log($_G[gp_uidarray][$key], 'deletepoll',$_G[gp_ridarray][$key]);//积分
			group_add_empirical_by_setting($_G[gp_uidarray][$key],$_G[gp_fidarray][$key],"vote_delete");//经验值
	       hook_delete_resource($rid, "poll");
	   }
   }
   cpmsg_mgr("操作成功", "manage.php?mod=group&op=poll&method=search", "succeed");
}

include template("manage/group_poll");
