...
if($_G["gp_method"]=="search"){
   ...
   /* require_once libfile("function/org");
    ...
    $sql = "SELECT #fileds# FROM ".DB::Table("forum_thread")." ft,".DB::Table("common_resource")." cr,"
        ...
        ." WHERE cm.uid=cmp.uid AND cm.uid=cr.uid AND ft.tid=cr.rid AND cr.rtype='topic'";
    ...
    if(!empty($_G[gp_content])){
        ...
        $sql = $sql." AND cr.rid IN(".implode(",", $topicids).")";
    }
    ...
    $url = "manage.php?mod=group&op=topic&method=search&code=true&usernames=$usernames&usernames_uids=$usernames_uids&content=$content&starttime=$starttime&endtime=$endtime";
  ...
  require_once libfile("function/credit");
   if(!empty($_G[gp_ridarray])){
	   foreach($_G[gp_ridarray] as $key => $rid){
			...
			credit_create_credit_log($_G[gp_uidarray][$key], 'deletetopic',$_G[gp_ridarray][$key]);//积分
			group_add_empirical_by_setting($_G[gp_uidarray][$key],$_G[gp_fidarray][$key],"topic_delete");//经验值
	       hook_delete_resource($rid, "topic");
	   }
   }
   cpmsg_mgr("操作成功", "manage.php?mod=group&op=topic&method=search", "succeed");
}

include template("manage/group_topic");
