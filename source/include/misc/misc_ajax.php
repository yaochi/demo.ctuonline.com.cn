 ...
 *
 *      $Id: misc_ajax.php 11030 2010-05-20 04:32:17Z xupeng $
 */
...
/*	$cid = empty($_GET['cid'])?0:intval($_GET['cid']);
	$comment_idtype = empty($_GET['idtype'])?'':$_GET['idtype'];
	$update_table = empty($_GET['update_table'])?'':$_GET['update_table'];
	$update_id = empty($_GET['update_id'])?'':$_GET['update_id'];
	...
	if($_G[uid]){
		if($cidsql){
			$cidsql.=" AND authorid='$_G[uid]'";
		}else{
			$cidsql.=" authorid='$_G[uid]'";
		}		
	}

	$list = array();
	$query = DB::query("SELECT * FROM ".DB::table('home_comment')." WHERE $cidsql ORDER BY dateline DESC LIMIT 0,1");
	while ($value = DB::fetch($query)) {
...
}elseif($op=='extraresource'){
	$extype=$_GET['extype'];
	$classid=$_GET['classid'];
	$orgid=$_GET['orgid'];
	$lecid=$_GET['lecid'];
	if($extype=='class'){
		$class=DB::fetch_first("select * from ".DB::TABLE("extra_class")." where id=".$classid);
	}else if($extype=='org'){
		$org=DB::fetch_first("select * from ".DB::TABLE("extra_org")." where id=".$orgid);
	}else{
		$lec=DB::fetch_first("select * from ".DB::TABLE("extra_lecture")." where id=".$lecid);
	}
}
