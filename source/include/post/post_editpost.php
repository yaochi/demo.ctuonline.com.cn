 ...
 *
 *      $Id: post_editpost.php 11753 2010-06-12 06:44:03Z tiger $
 */
...
if(!submitcheck('editsubmit')) {
	...
	if($isfirstpost) {
		...
		if($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $thread['authorid'] == $_G['uid'])) {
			$query = DB::query("SELECT polloptionid, displayorder, polloption, multiple, allvote, visible, maxchoices, expiration, overt FROM ".DB::table('forum_polloption')." AS polloptions LEFT JOIN ".DB::table('forum_poll')." AS polls ON polloptions.tid=polls.tid WHERE polls.tid ='$_G[tid]' ORDER BY displayorder");
			while($temppoll = DB::fetch($query)) {
				$poll['multiple'] = $temppoll['multiple'];
				$poll['visible'] = $temppoll['visible'];
				$poll['allvote'] = $temppoll['allvote'];
	...
	if($_G['group']['allowpostattach'] || $_G['group']['allowpostimage']) {
		...
		//kindeditor编辑器编辑展示图片tag转换 caimm
		$findtag = explode('[attachimg]',$postinfo['message']);
        for($i=1;$i<count($findtag);$i++){
            $st = $findtag[$i];
            $str = explode('[/attachimg]',$st);
            $aid = $str[0];
            $info=DB::query("SELECT * FROM ".DB::table('forum_attachment')." where aid = ".$str[0]);
            $rt=DB::fetch($info);
            if($rt){
                 $k=$aid;
                 $postinfo['message'] = str_replace('[attachimg]'.$k.'[/attachimg]','<img src="data/attachment/forum/'.$rt[attachment].'" />',$postinfo['message']);
             }
        }
        //end
	}
	...
	$redirecturl = "forum.php?mod=viewthread&tid=$_G[tid]&page=$_G[gp_page]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]&per=$_G[gp_per]&ordertype=$_G[gp_ordertype]&$_extra=$extra".($vid && $isfirstpost ? "&vid=$vid" : '')."#pid$pid";
	...
	if(empty($_G['gp_delete'])) {
		...
		if($isfirstpost) {
			...
			if($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $isorigauthor) && !empty($_G['gp_polls'])) {
				...
				if($pollarray['options']) {
					...
					$pollarray['allvote'] = empty($_G['gp_votepoll']);
					...
					DB::query("UPDATE ".DB::table('forum_poll')." SET multiple='$pollarray[multiple]', visible='$pollarray[visible]',allvote='$pollarray[allvote]', maxchoices='$pollarray[maxchoices]', expiration='$pollarray[expiration]', overt='$pollarray[overt]', pollpreview='$polloptionpreview' WHERE tid='$_G[tid]'", 'UNBUFFERED');
				} else {
					$polladd = ', special=\'0\'';
				 ...
				 */
				//begin
				$oldrewardprice=intval($_POST['oldrewardprice']);
				$newrewardprice=intval($_POST['rewardprice']);
				if($newrewardprice>$oldrewardprice){
					group_add_empirical($thread['authorid'],$thread['fid'],$oldrewardprice-$newrewardprice);//更新用户在专区的经验值
				}
			    $new_category_id=$_POST['category'];//修改
			    if($new_category_id&&$new_category_id!=$thread['category_id']){
			    	DB::query("UPDATE pre_forum_thread SET category_id='".$new_category_id."' WHERE tid='".$thread['tid']."';");
			    }
				//end
			...
			$new_category_id=$_POST['category'];//修改
		    if($new_category_id&&$new_category_id!=$thread['category_id']){
		    	DB::query("UPDATE pre_forum_thread SET category_id='".$new_category_id."' WHERE tid='".$_G[tid]."';");
		    }
		...
		if($_G['setting']['editedby'] && (TIMESTAMP - $orig['dateline']) > 60 && $_G['adminid'] != 1) {
			...
			$message = lang('forum/misc', $htmlon ? 'post_edithtml' : 'post_edit', array('editor' => user_get_user_name_by_username($editor), 'edittime' => $edittime)) . $message;
		}
		...
		if($thread['special'] == 0){
			hook_delete_resource($_G[tid],'topic');
		}
		if($thread['special'] == 1){
			hook_delete_resource($_G[tid],'poll');
		}
		...
		if($thread['special'] == 3 && $isfirstpost) {
			...
			/**
			* added by fumz,2010-8-11 10:44:20
			* 调用积分接口
			*/
			hook_delete_resource($_G[tid],'qbar');
			//begin
			require_once libfile("function/group");
			group_add_empirical_by_setting($thread['authorid'],$_G['fid'],"quesiton_delete");
		...
		if(empty($isfirstpost)){
			hook_delete_resource($pid,'group_pcomment');
			$pcommentquery=DB::query("SELECT id FROM ".DB::table(forum_postcomment)." WHERE pid='$pid'");
			while($pcomment=DB::fetch($pcommentquery)){
				hook_delete_resource($pcomment[id],'group_ppcomment');
			}
			DB::query("DELETE FROM ".DB::table(forum_postcomment)." where pid='$pid'");
	...
	if($_G['forum_auditstatuson']) {
		...
		if(!empty($_G['gp_delete']) && $isfirstpost) {
			...
			showmessage('post_edit_delete_succeed', "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$special", $param);
