...
if(!submitcheck('topicsubmit', 0, $seccodecheck, $secqaacheck)) {
    ...
    $info = common_category_is_other($_G["gp_fid"], $plugin_name);
    $is_enable_category = false;
    if($info && $info["state"]=='Y'){
        $is_enable_category = true;
        $categorys = common_category_get_category($_G["gp_fid"], $plugin_name);
    }
	$required=common_category_is_required($_G["gp_fid"], $plugin_name);
	$empiricalvalue=group_get_empirical($_G['uid'],$_G['fid']);//added by fumz,2010-7-30 13:37:10查询用户在当前专区的经验值
	...
	if($special == 1) {
		...
		foreach($polloption as $key => $value) {
			$pollarray['options'][$key]	= str_replace('　',' ',$pollarray['options'][$key]);
		...
		$pollarray['allpoll'] = empty($_G['gp_votepoll']);
		...
		//此处代码做了一定的修改by fumz ，2010-7-30 14:39:43
		//begin
		$empiricalvalue=group_get_empirical($_G['uid'],$_G['fid']);//added by fumz,2010-7-30 13:37:10查询用户在当前专区的经验值
		if($rewardprice > $empiricalvalue) {
			showmessage('reward_credits_please');
		} elseif($rewardprice<0) {
			showmessage('reward_credits_please');
		}
		$price = $rewardprice;
		$newempiricalvalue=$rewardprice;//发表问题填入的经验值
		$newempiricalvalue=0-$newempiricalvalue;
		if($newempiricalvalue!=0){
			group_add_empirical($_G['uid'],$_G['fid'],$newempiricalvalue);//更新用户在专区的经验值
		}
		...
		//feed_add('static/image/plugins/qbar.gif','');//产生动态,2010-8-11 9:43:56,discuz中有
    ...
    $plugin_name = $_G["gp_plugin_name"];
    $info = common_category_is_other($_G["gp_fid"], $plugin_name);
    if($info && $info["state"]=='Y' && $info["required"]=='Y' && $category_id==0){
        $url = "forum.php?mod=post&action=newthread&fid=".$_G["fid"]."&plugin_name=".$_G["gp_plugin_name"]."&plugin_op=".$_G["gp_groupmenu"]."&special=".intval(getgpc('special'));
        showmessage("请输入类别");
    }

	DB::query("INSERT INTO ".DB::table('forum_thread')." (fid, posttableid, readperm, price, typeid, sortid, author, authorid, subject, dateline, lastpost, lastposter, displayorder, digest, special, attachment, moderated, status, isgroup, category_id)
		VALUES ('$_G[fid]', '$posttableid', '$readperm', '$price', '$typeid', '$sortid', '$author', '$_G[uid]', '$subject', '$_G[timestamp]', '$_G[timestamp]', '$author', '$displayorder', '$digest', '$special', '0', '$moderated', '$thread[status]', '$isgroup', '$category_id')");
	$tid = DB::insert_id();
	...
	if($special==0){
		...
		credit_create_credit_log($_G['uid'], 'createtopic',$tid);//调用系统积分接口，实现提问时的积分功能,2010-8-20 14:04:46 yyang
		hook_create_resource($tid, "topic",$_G['gp_fid']);
		//经验值
		require_once libfile("function/group");
		group_add_empirical_by_setting($_G['uid'],$_G['gp_fid'],'topic_publish', $tid);
	}
	...
	if($special == 3 && $_G['group']['allowpostreward']) {
		 ...
		 * added by fumz
		 */
		 hook_create_resource($tid, "qbar",$_G[fid]);
		//begin
		require_once libfile("function/credit");
		credit_create_credit_log($_G['uid'], 'createquestion',$tid);//调用系统积分接口，实现提问时的积分功能,2010-8-11 9:44:14
		//end
	}
	...
	if($special == 1) {
		...
		hook_create_resource($tid, "poll",$_G[fid]);
		require_once libfile("function/credit");
		credit_create_credit_log($_G['uid'], 'createpoll',$tid);//调用系统积分接口，实现提问时的积分功能,2010-8-20 14:04:46 yyang

		//经验值
		require_once libfile("function/group");
		group_add_empirical_by_setting($_G['uid'],$_G[fid],'vote_publish', $tid);

		DB::query("INSERT INTO ".DB::table('forum_poll')." (tid, multiple, visible, allvote, maxchoices, expiration, overt, pollpreview)
			VALUES ('$tid', '$pollarray[multiple]', '$pollarray[visible]', '$pollarray[allvote]', '$pollarray[maxchoices]', '$pollarray[expiration]', '$pollarray[overt]', '$polloptionpreview')");
	...
	//删除message中图片已经删除的图片tag
	$findtag = explode('[attach]',$message);
        for($i=1;$i<count($findtag);$i++){
            ...
            if($rt == 0){
                 $k=$del_aid;
                 $message = str_replace('[attach]'.$k.'[/attach]','',$message);
            }
            }
        }
	//end
    ...
    if($special===0||$special===1||$special===3){
        require_once libfile("function/post");
        post_forum_inc_threads($_G[fid]);
    }
	...
	if($modnewthreads) {
		...
		showmessage('post_newthread_mod_succeed', "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]", $param);
	} else {
		...
		if($special==3){//如果用户发表的是提问，added by fumz 2010-7-30 16:08:13
			showmessage('post_newthread_succeed', "forum.php?mod=viewthread&tid=$tid&special=$special&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]", $param);
		}
		showmessage('post_newthread_succeed', "forum.php?mod=viewthread&tid=$tid&extra=$extra&special=$special&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]", $param);
