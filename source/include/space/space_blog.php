 ...
 *
 *      $Id: space_blog.php 11789 2010-06-13 02:53:32Z zhengqingpeng $
 */
...
if($id) {
	...
	$feedarray=DB::fetch_first("select * from ".DB::TABLE("home_feed")." where icon='blog' and id=".$blog[blogid]);
	...
	if($count) {
		...
		if(empty($list) && empty($cid)) {
			$count = getcount('home_comment', array('id'=>$feedid, 'idtype'=>'feed'));
	...
	if($need_count) {
		...
		$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('home_blog')." b WHERE $wheresql and b.anonymity=0"),0);
		if($count) {
			$query = DB::query("SELECT bf.*, b.* FROM ".DB::table('home_blog')." b $f_index
				LEFT JOIN ".DB::table('home_blogfield')." bf ON bf.blogid=b.blogid
				WHERE $wheresql  and b.anonymity=0
		...
		while ($value = DB::fetch($query)) {
			if(ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {
				if($value['friend'] == 4) {
					...
					$value['message'] = getstr($value['message'], 0, 0, 0, 0, 0, -1);
				}
				$value['message'] = preg_replace("/&[a-z]+\;/i", '', $value['message']);
				$value['message'] = cutstr($value['message'], $summarylen);
...
if( checkIsOfficial($space['uid']) ){
	//是官方博客
	if($page <=1 && empty($_GET['classid']) && $diymode)//显示置顶日志 . 当为第二页、只查看某一分类、个人中心时不去获得置顶日志。
	{
		$topblog_ids = array();//存放置顶id 
		$query = DB::query("SELECT bf.*, b.* FROM ".DB::table('home_blog')." b LEFT JOIN ".DB::table('home_blogfield')." bf ON bf.blogid=b.blogid WHERE  b.uid='$space[uid]' AND b.istop=1 ORDER BY topdateline DESC LIMIT 0,2");
		while ($value = DB::fetch($query)) {
			if(ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {
				if($value['friend'] == 4) {
					$value['message'] = $value['pic'] = '';
	...
	//获取链接列表
	$query = DB::query("SELECT * FROM ".DB::table('home_official_link')." order by dateline desc");
	while($value=DB::fetch($query))
	{
		$linkarr[] = $value;
	}
	include_once template("diy:home/space_blog_official_list");
}else{
