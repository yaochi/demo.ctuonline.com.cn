...
if($op == 'send' || $op == 'reply') {
	...
	if(submitcheck('pokesubmit')) {
		...
		credit_create_credit_log($_G['uid'], 'poke', $tospace['uid']); 
		

		showmessage('poke_success', dreferer(), array('username' => $tospace['realname'], 'uid' => $uid), array('showdialog'=>1, 'showmsg' => true, 'closetime' => 1));
	...
	if($value = DB::fetch($query)) {
		...
		$value['fromusername']=user_get_user_name_by_username($value['fromusername']);
