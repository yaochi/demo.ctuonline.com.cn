...
$operation = in_array($_GET['op'], array('base', 'feed', 'filter', 'getgroup','getrealgroup')) ? trim($_GET['op']) : 'base';
...
} elseif ($operation == 'getrealgroup') {

	$gid = empty($_GET['gid'])?0:intval($_GET['gid']);
	$rusers=array();
	$query = DB::query("SELECT fusername FROM ".DB::table('home_friend')." WHERE uid='$_G[uid]' AND gid='$gid'");
	while ($value = DB::fetch($query)) {
		$rusers[]=user_get_user_name_by_username($value['fusername']);
	}
	$rustr = empty($rusers)?'': dhtmlspecialchars(implode(' ', $rusers));
	showmessage($rustr, '', array(), array('msgtype' => 3, 'handle'=>false));
} else {
