...
$myfields = array('uid','realname','gender','birthyear','birthmonth','birthday','birthprovince','birthcity','resideprovince','residecity', 'residedist', 'residecommunity');
...
if(!empty($_GET['searchsubmit']) || !empty($_GET['searchmode'])) {
	if(!empty($_GET['org']) ){
		...
		$orgid=get_org_by_user($_G['username']);
		$values=getUserByOrg($orgid[id]);
		$list=array();
		foreach ($values as $value) {
			$value['uid']=$value['id'];
			$list[$value['uid']] = $value;
		}
	}else if(!empty($_GET['station'])){
		require_once libfile('function/org');
		$stations=getStationByUser($_G['uid']);
		$list=array();
		if(empty($stations)){
			showmessage('您的岗位信息不全，请与hr联系！','home.php?mod=spacecp&ac=search');
		}else{
			foreach ($stations as $station) {
				$values=org_station_by_user($station[s_id]);
					foreach ($values as $value) {
						$value['uid']=$value['id'];
						$list[$value['uid']] = $value;
					}
	...
	}else if(!empty($_GET['depart'])){
		require_once libfile('function/org');
		$values=getUserByOrg($_GET['orgname_input_id']);
		$list=array();
		if(empty($values)){
			showmessage('该部门下没有用户！','home.php?mod=spacecp&ac=search');
		}else{
			foreach ($values as $value) {
				$value['uid']=$value['id'];
				$list[$value['uid']] = $value;
		...
		foreach ($myfields as $fkey) {
			...
			if($_GET[$fkey]) {
				if($fkey=='realname'){
					$havefield = 1;
					$wherearr[] = "sf.$fkey LIKE '%".$_GET[$fkey]."%'";
				}else{
					$havefield = 1;
...
foreach($list as $item){
    $realnameuids[] = $item[uid];
}
$realnames =  user_get_user_realname($realnameuids);
