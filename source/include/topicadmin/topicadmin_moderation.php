...
$allow_operation = array('delete', 'approve', 'highlight', 'open', 'close', 'stick', 'digest', 'bump', 'down', 'recommend', 'type', 'move', 'recommend_group');
...
if($tids = dimplode($_G['gp_moderate'])) {
	...
	while($thread = DB::fetch($query)) {
		if($thread['closed'] > 1 && $operation && !in_array($operation, array('delete', 'approve', 'highlight', 'stick', 'digest', 'bump', 'down'))) {
			continue;
		}
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	case '3'://added by fumz,2010-8-3 17:33:45
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=qbar&plugin_op=groupmenu";
		break;
	default:
		$_G['referer'] = "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$special&".str_replace('amp;', '&', rawurldecode(getgpc('listextra')));
		break;
}
...
if(!submitcheck('modsubmit')) {
	...
	if($_G['gp_optgroup'] == 1 && $single) {
		...
		$pid=$_REQUEST['pid'];
		$categoryhtml="";
		$query = DB::query("SELECT * FROM ".DB::table('common_category')." WHERE pid='$pid' AND fid='$fid' ORDER BY displayorder");
		while($type = DB::fetch($query)) {
			$categoryhtml.="<option value='$type[id]'>$type[name]</option>";
		}		
		//end			
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				if($sticklevel!=0){
					...
					foreach($threadlist as $thread) {
						if ($thread['special']=='0') {
							group_add_empirical_by_setting($thread['authorid'], $_G['fid'], 'topic_top', $thread[tid]);
						}
					}
				}
				foreach($threadlist as $thread){
				require_once libfile("function/credit");
				credit_create_credit_log($thread['authorid'], 'top',$thread['tid']);//积分
					if($thread['special']=='3'){
						if($sticklevel==$thread[displayorder]){
						}else if($sticklevel=='0'){
							notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销置顶了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>置顶了，赶快去看看吧', array(), 1);
						}
					}elseif($thread['special']=='0'){
						if($sticklevel==$thread[displayorder]){
						}else if($sticklevel=='0'){
							notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销置顶了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>置顶了，赶快去看看吧', array(), 1);
						}
					}elseif($thread['special']=='1'){
						if($sticklevel==$thread[displayorder]){
						}else if($sticklevel=='0'){
							notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销置顶了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销置顶了，赶快去看看吧', array(), 1);
						}
					}
				...
				//	加精经验值
					...
					foreach($threadlist as $thread) {
						if ($thread['special']=='0') {
							group_add_empirical_by_setting($thread['authorid'], $_G['fid'], 'topic_competitive', $thread[tid]);
						}
					}
				}
				foreach($threadlist as $thread){
				require_once libfile("function/credit");
				credit_create_credit_log($thread['authorid'], 'digest',$thread['tid']);//积分
				if($thread['special']=='3'){
						if($digestlevel==$thread[digest]){
						}else if($digestlevel=='0'){
							notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销加精了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>加精了，赶快去看看吧', array(), 1);
						}
					}elseif($thread['special']=='0'){
						if($digestlevel==$thread[digest]){
						}else if($digestlevel=='0'){
							notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销加精了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>加精了，赶快去看看吧', array(), 1);
						}
					}elseif($thread['special']=='1'){
						if($digestlevel==$thread[digest]){
						}else if($digestlevel=='0'){
							notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>撤销加精了，赶快去看看吧', array(), 1);
						}else{
							notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>加精了，赶快去看看吧', array(), 1);
						}
					}
				}
				foreach($threadlist as $thread) {
				...
				foreach($threadlist as $thread) {
					if($thread['special']==3){
					hook_delete_resource($thread['tid'],'qbar');
						group_add_empirical_by_setting($thread['authorid'],$_G['fid'],"quesiton_delete");//用户问题被删除时更改经验值
						credit_create_credit_log($thread['authorid'], 'deletequestion',$thread['tid']);
						notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的“'.$thread[subject].'”的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>删除了', array(), 1);
					} else if($thread['special']==0){
						//begin added by SK
						group_add_empirical_by_setting($thread['authorid'],$_G['fid'],"topic_delete");//话题被删除时更改经验值				
						hook_delete_resource($thread['tid'],'topic');
						credit_create_credit_log($thread['authorid'], 'deletetopic',$thread['tid']);//积分
						notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的“'.$thread[subject].'”的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>删除了', array(), 1);
						//end
						//require_once libfile("function/post");
                        //post_forum_dec_threads($_G[fid]);
                    }else if($thread['special']==1){
						hook_delete_resource($thread['tid'],'poll');
						credit_create_credit_log($thread['authorid'], 'deletepoll',$thread['tid']);//积分
						group_add_empirical_by_setting($thread['authorid'],$_G['fid'],"vote_delete");//经验值
						notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的“'.$thread[subject].'”的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>删除了', array(), 1);
                    }
				}	
				//end
				...
				DB::query("UPDATE ".DB::table('forum_thread')." SET closed='1', moderated='1',question_status='Y' WHERE tid IN ($moderatetids)");
				...
				DB::query("UPDATE ".DB::table('forum_thread')." SET closed='0', moderated='1',question_status='N' WHERE tid IN ($moderatetids)");
				DB::query("UPDATE ".DB::table('forum_threadmod')." SET status='0' WHERE tid IN ($moderatetids) AND action IN ('CLS','OPN','ECL','UCL','EOP','UEO')", 'UNBUFFERED');
				...
				if(!$_G['group']['allowedittypethread']&&!$_G['group']['allowdelpost']) {
					showmessage('undefined_action');
				}
				...
				$category_id=$_REQUEST['category'];//added by fumz,2010-8-9 10:27:26;分类
				$oldcategory=DB::fetch_first("SELECT name FROM ".DB::table("common_category")." WHERE id=".$thread[category_id]);
				$newcategory=DB::fetch_first("SELECT name FROM ".DB::table("common_category")." WHERE id='".$category_id."'");
				DB::query("UPDATE ".DB::table('forum_thread')." SET typeid='$_G[gp_typeid]',category_id='$category_id', moderated='1' WHERE tid IN ($moderatetids)");//modified by fumz,2010-8-9 10:25:30;添加了category_id='$category_id'
				if($oldcategory[name]){
					if($_G[gp_typeid]==1){
						notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>从“'.$oldcategory[name].'”分类下移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
					}elseif($_G[gp_typeid]==3){
						notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>从“'.$oldcategory[name].'”分类下移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
					}elseif($_G[gp_typeid]==0){
						notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>从“'.$oldcategory[name].'”分类下移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
					}else{}
				}else{
						if($_G[gp_typeid]==1){
						notification_add($thread['authorid'],"poll",'[投票]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的投票被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
						}elseif($_G[gp_typeid]==3){
						notification_add($thread['authorid'],"reward",'[提问吧]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'&plugin_name=qbar">“'.$thread[subject].'”</a>的提问被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
						}elseif($_G[gp_typeid]==0){
						notification_add($thread['authorid'],"thread",'[话题]您<a href="forum.php?mod=group&fid='.$_G['fid'].'">“'.$_G[forum][name].'”</a>的<a href="forum.php?mod=viewthread&tid='.$thread[tid].'&fid='.$_G['fid'].'">“'.$thread[subject].'”</a>的话题被<a href="home.php?mod=space&uid='.$_G['member']['uid'].'">['.user_get_user_name_by_username($_G['member']['username']).']</a>移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
