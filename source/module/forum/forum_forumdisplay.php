 ...
 *
 *      $Id: forum_forumdisplay.php 11753 2010-06-12 06:44:03Z tiger $
 */
...
if(empty($_G["gp_action"]) && $_G["mod"]=="forumdisplay"){
	if($_GET['special']==1){
    	dheader("Location: forum.php?mod=forumdisplay&action=list&fid=$_G[fid]&special=1&plugin_name=poll&plugin_op=groupmenu&page=".$_G['gp_page']);
	}else{
		if (!empty($_GET['category_id'])) {
			$category_id = "&category_id=".$_GET['category_id'];
		}
		dheader("Location: forum.php?mod=forumdisplay&action=list&fid=$_G[fid]&special=0&plugin_name=topic&plugin_op=groupmenu&page=".$_G['gp_page'].$category_id);
	}
...
$_G['gp_special'] = isset($_G['gp_special']) ? $_G['gp_special'] : '';
...
if($_G["fid"]){
    ...
    if(common_category_is_enable($_G["gp_fid"], $pluginid)){
        $is_enable_category = true;
        $categorys = common_category_get_category($_G["gp_fid"], $pluginid);
    }
	if(common_category_is_prefix($_G["gp_fid"], $pluginid)){
		$prefix=true;
	}
    ...
    $query = DB::query("SELECT ffa.*, fff.extra FROM " . DB::table("forum_forum_activity") . " ffa, ".DB::table("forum_forumfield")." fff WHERE ffa.fid=fff.fid AND fff.fid=" . $_G["fid"]);
    $_G["activity"] = DB::fetch($query);
    if ($_G["activity"]) {
        $_G["activity"][live_id] = unserialize($_G["activity"][live_id]);
        $extra = unserialize($_G[activity][extra]);
        ...
        $query = DB::query("SELECT * FROM ".DB::table("forum_groupuser")." WHERE fid=$_G[fid] ORDER BY joindateline DESC LIMIT 8");
...
$filterfield = array('digest', 'recommend', 'typeid', 'sortid', 'special', 'dateline', 'specialtype', 'author', 'reply', 'view', 'lastpost', 'heat', 'page','voters');
...
if(!empty($_G['gp_orderby']) && in_array($_G['gp_orderby'], array('lastpost', 'dateline', 'replies', 'views', 'recommends', 'heats','voters'))) {
...
if(empty($_G['gp_filter']) && empty($_GET['category_id']) && empty($_G['gp_sortid']) && empty($_G['gp_special']) && empty($_G['gp_archiveid']) && empty($_G['forum']['archive'])) {
	$_G['forum_threadcount'] = $_G['forum']['threads'];
} else {
	if(!empty($_GET['category_id'])) {
		$cate_sql = " AND category_id=".$_GET['category_id'];
	} else {
		$cate_sql = "";
	}
	$_G['forum_threadcount'] = DB::result_first("SELECT COUNT(*) FROM ".DB::table($threadtable)." WHERE fid='{$_G['fid']}' AND special='{$_G['gp_special']}' $filteradd AND displayorder>='0'".$cate_sql);
}
...
if (!empty($_GET['category_id'])) {
	$add_cate_url = "&category_id=".$_GET['category_id'];
}
$multipage = multi($_G['forum_threadcount'], $_G['tpp'], $page, "forum.php?mod=forumdisplay&special=".$_G['gp_special']."&fid=$_G[fid]&plugin_name=topic&plugin_op=groupmenu".($_G['gp_filter'] ? "&filter=$_G[gp_filter]" : '')."$forumdisplayadd[orderby]$forumdisplayadd[page]$multipage_archive".$add_cate_url, $_G['setting']['threadmaxpages']);
...
if(isset($special)){
    $filteradd .= ' AND special='.$special;    
}
if(isset($_GET["category_id"]) && $_GET["category_id"]!=0){
    $filteradd .= ' AND category_id='.$_GET["category_id"];
}
if(($start_limit && $start_limit > $stickycount) || !$stickycount || $filterbool) {
		$querysticky = '';
	if($_G[gp_orderby]=='voters'){
		$query = DB::query("SELECT t.* FROM ".DB::table($threadtable)." t,".DB::table("forum_poll")." p
			WHERE t.fid='{$_G['fid']}' AND t.tid=p.tid $filteradd AND $displayorderadd
			ORDER BY t.displayorder DESC, p.$_G[gp_orderby] $_G[gp_ascdesc]
...
while(($querysticky && $thread = DB::fetch($querysticky)) || ($query && $thread = DB::fetch($query))) {
	...
	$thread['realname'] = user_get_user_name($thread[authorid]);
	if($thread[lastposter]=='-1'){
		$thread['lastpostername']='匿名';
	}else{
		$thread['lastpostername'] = user_get_user_name_by_username($thread[lastposter]);
	}
	...
	if($thread['special']==1){
		$pollquery = DB::query("SELECT * FROM ".DB::table('forum_poll')." WHERE tid =".$thread['tid']);
		while($value = DB::fetch($pollquery)) {
	...
	$_G['forum_threadlist'][$thread['tid']] = $thread;

}
if($threadids){
	if($special=='0'){
	$query=DB::query("select anonymity,id from ".DB::TABLE("home_feed")." where icon='thread' and idtype!='feed' and id in (".implode(',',$threadids).")");
	}elseif($special=='1'){
		$query=DB::query("select anonymity,id from ".DB::TABLE("home_feed")." where icon='poll' and idtype!='feed' and id in (".implode(',',$threadids).")");
	}elseif($special=='3'){
		$query=DB::query("select anonymity,id from ".DB::TABLE("home_feed")." where icon='reward' and idtype!='feed' and id in (".implode(',',$threadids).")");
	}
	while($value=DB::fetch($query)){
		if($value[anonymity]>0){
			include_once libfile('function/repeats','plugin/repeats');
			$_G['forum_threadlist'][$value['id']]['repeats']=getforuminfo($value[anonymity]);
		}elseif($value[anonymity]=='-1'){
			
			$_G['forum_threadlist'][$value['id']]['anonymity']='-1';
			if($_G['forum_threadlist'][$value['id']][replies]=='0'){
				$_G['forum_threadlist'][$value['id']][lastpostername]='匿名';
				$_G['forum_threadlist'][$value['id']][lastposterenc]='0';
			}
		}
	}
}
...
if(!empty($_G['forum']['threadsorts']['templatelist']) && $_G['forum']['status'] != 3) {
    ...
    if($_G["forum"]["type"]=="activity"){
        $template = 'diy:group/activity:'.$_G['fid'];
    }
}
