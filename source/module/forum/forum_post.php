 ...
 *
 *      $Id: forum_post.php 11454 2010-06-03 04:12:56Z liulanbo $
 */
...
$currentpos=getgpc('currentpos');//added by fumz,当前用户浏览的页面
...
if($_G['forum']['status'] == 3) {
	...
	$cancreate=group_get_plugin_available($_G['forum']['fid']);
	if($status == -1) {
		showmessage('forum_not_group', 'index.php');
	} elseif($status == 1) {
		showmessage('forum_group_status_off');
	} elseif($status == 2) {
		showmessage('forum_group_noallowed', "forum.php?mod=group&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]");
	} elseif($status == 3) {
		showmessage('forum_group_moderated', "forum.php?mod=group&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]");
	} elseif($status == 4) {
		/*if($special==3){//added by fumz,2010-8-18 16:14:46
//			if($currentpos=='viewquestion'){
				//$tid=getgpc('tid');
				if($cancreate['qbar']['auth_group']!='全部'){
				showmessage('forum_group_not_groupmember', "forum.php?mod=group&fid=$_G[fid]", array(), array('showmsg' => 1));
				}
//			}else{
//				showmessage('forum_group_not_groupmember', "forum.php?mod=group&plugin_name=qbar&plugin_op=groupmenu&action=plugin&fid=$_G[fid]", array(), array('showmsg' => 1));
//			}
		}elseif($special==1){
			if($cancreate['poll']['auth_group']!='全部'){
				showmessage('forum_group_not_groupmember', "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]", array(), array('showmsg' => 1));
			}
		}else{
			if($cancreate['topic']['auth_group']!='全部'){
				showmessage('forum_group_not_groupmember', "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]", array(), array('showmsg' => 1));
...
if($_G['forum']['status'] == 3) {
	...
	if($_G['forum'][type]=='sub'){
		//$navigation = ' &rsaquo; <a href="group.php">'.专区.'</a> '.
		$navigation=get_groupnav($_G['forum']);
	}elseif($_G['forum'][type]=='activity'){
		//$navigation=' &rsaquo; <a href="group.php">'.专区.'</a> ';
		//$navigation.=" &rsaquo; <a href='forum.php?mod=group&fid=".$_G[parent][fid]."'>"."1111"."</a>  ";
		if($_G['forum']['fup']){
        	$sql = "SELECT * FROM ".DB::table("forum_forum")." WHERE fid=".$_G['forum']['fup'];
        	$query = DB::query($sql);
        	$_G['parent'] = DB::fetch($query);
		}
		//$navigation.=" &rsaquo; <a href='forum.php?mod=activity&fup=".$_G['forum']['fup']."&fid=".$_G['forum']['fid']."'>".活动.'</a>';
		//$navigation.=get_activitynav($_G['forum']);
		$navigation.=get_groupcreatenav($_G['forum']);
	}
...
if($_G['forum']['password'] && $_G['forum']['password'] != $_G['cookie']['fidpw'.$_G['fid']]) {
	showmessage('forum_passwd', "forum.php?mod=forumdisplay&fid=$_G[fid]&special=$_G[gp_special]&plugin_name=$_G[gp_plugin_name]&plugin_op=$_G[gp_plugin_op]");
}
