 ...
 *
 *      $Id: forum_topicadmin.php 11288 2010-05-28 02:00:05Z monkey $
 */
...
if($_G['gp_action'] == 'moderate') {
...
} elseif($_G['gp_action'] == 'noticemoderate') {

	require_once libfile('notice/moderation', 'plugin');

} elseif($_G['gp_action'] == 'extraresourcemoderate') {

	require_once libfile('extraresource/moderation', 'plugin');

} elseif($_G['gp_action'] == 'resourcelistmoderate') {
	
	require_once libfile('resourcelist/moderation', 'plugin');

} elseif($_G['gp_action'] == 'activitymoderate') {

	require_once libfile('activity/moderation', 'plugin');

} elseif($_G['gp_action'] == 'questionarymoderate') {

	require_once libfile('questionary/moderation', 'plugin');


} elseif($_G['gp_action'] == 'lecturermoderate') {

	require_once libfile('lecturer/moderation', 'plugin');

} elseif($_G['gp_action'] == 'shresourcelistmoderate') {
	
	require_once libfile('shresourcelist/moderation', 'plugin');

} elseif($_G['gp_action'] == 'lecturerecordmoderate') {

	require_once libfile('lecturerecord/moderation', 'plugin');

} elseif($_G['gp_action'] == 'lecturermanagemoderate') {

	require_once libfile('lecturermanage/moderation', 'plugin');

} elseif($_G['gp_action'] == 'selectionmoderate') {

	require_once libfile('selection/moderation', 'plugin');

} elseif($_G['gp_action'] == 'shlecturermoderate') {

	require_once libfile('shlecturer/moderation', 'plugin');
	...
	if(!submitcheck('modsubmit')) {
		...
		DB::query("UPDATE ".DB::table('forum_thread')." SET special='0', price='0' WHERE tid='$thread[tid]'", 'UNBUFFERED');//modified by fumz,2010-8-19 15:06:54
		...
		//showmessage('您的提问已经取消了',"forum.php?mod=group&action=plugin&fid=$thread['fid']&plugin_name=qbar&plugin_op=groupmenu");
	}
