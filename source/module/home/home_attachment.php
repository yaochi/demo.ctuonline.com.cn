...
if($_G['setting']['attachexpire']) {
	...
	if(empty($k) || empty($t) || $k != $authk || TIMESTAMP - $t > $_G['setting']['attachexpire'] * 3600) {
		$aid = intval($aid);
		if($attach = DB::fetch_first("SELECT isimage FROM ".DB::table('forum_attachment')." WHERE aid='$aid'")) {
			if($attach['isimage']) {
...
if(!empty($aid) && is_numeric($aid)) {
	$attach = DB::fetch_first("SELECT * FROM ".DB::table('home_attachment')." WHERE aid='$aid'");
	$attachexists = TRUE;
}
...
if(empty($_G['gp_noupdate'])) {
	if($_G['setting']['delayviewcount'] == 2 || $_G['setting']['delayviewcount'] == 3) {
		...
		if(substr(TIMESTAMP, -1) == '0') {
			...
			updateviews('home_attachment', 'aid', 'downloads', $_G['home_logfile']);
		}
		...
		DB::query("UPDATE ".DB::table('home_attachment')." SET downloads=downloads+'1' WHERE aid='$aid'", 'UNBUFFERED');
	}
}
