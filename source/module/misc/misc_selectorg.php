...
global $_G;
$date=date("Ymd",time());
$mykey=md5("api_grouptree_"."$date");
if($_GET[key]!=$mykey) {
    echo "Key Error!";
    exit();
}
$query = DB::query("SELECT data FROM " . DB::table("common_syscache") . " WHERE cname='orgtree_api'");
$data = DB::fetch($query);
...
if (!$data["data"]) {
    ...
    foreach ($allorg as $item) {
        ...
        if($parentId==-1){
            ...
            $t .= 'd.add(' . $item["id"] . ', ' . $parentId . ', "' . $item["name"] . '", "javascript:orgtreesel(' . $item["id"] . ', \'' . $item["name"]  .'\');");';
        }
    }
    $t .= 'document.getElementById("orgtree").innerHTML=d.toString();';
    $s .= $t;
    //写入cache
    DB::insert("common_syscache", array(cname => "orgtree_api", data => addslashes($t)));
}
...
include template('common/selectorg');
?>