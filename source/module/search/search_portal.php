 ...
 *
 *      $Id: search_portal.php 11092 2010-05-21 09:42:01Z zhaoxiongfei $
 */
...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		if($searchindex['id']) {
            ...
            $search = $search[0];
			$query = $sdb->query("SELECT aid FROM ".DB::table('portal_article_title')." WHERE title LIKE '%$search%' ORDER BY aid DESC LIMIT ".$_G['setting']['search']['portal']['maxsearchresults']);
			while($article = $sdb->fetch_array($query)) {
				$ids .= ','.$article['aid'];
				$num++;
			}
