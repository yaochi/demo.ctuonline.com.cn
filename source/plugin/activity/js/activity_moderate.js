...
function modactivitys(optgroup, operation) {
...
function tmodclick(obj) {
	...
	if(modclickcount > 0) {
		...
		while((obj = obj.offsetParent).id != 'activitylist') {
...
function tmodactivitys(optgroup, operation) {
	var checked = 0;
	var operation = !operation ? '' : operation;
	...
	if(!checked) {
		...
		showActivityWindow('mods', 'moderate', 'post');
	}
}

function showActivityWindow(k, url, mode, cache, menuv) {
