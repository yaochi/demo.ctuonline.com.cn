...
function modextraresources(optgroup, operation) {
...
function tmodclick(obj) {
	...
	if(modclickcount > 0) {
		...
		while((obj = obj.offsetParent).id != 'extraresourcelist') {
			top_offset += obj.offsetTop;
		}
		$('mdly').style.top = top_offset - 14 + 'px';
		$('mdly').style.display = '';
	} else {
		$('mdly').style.display = 'none';
	}
...
function tmodextraresources(optgroup, operation) {
	var checked = 0;
	var operation = !operation ? '' : operation;
	...
	if(!checked) {
		...
		showextraresourcerWindow('mods', 'moderate', 'post');
	}
}

function showextraresourcerWindow(k, url, mode, cache, menuv) {
