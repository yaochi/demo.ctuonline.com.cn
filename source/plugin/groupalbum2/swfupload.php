...
if($op == "finish") {
	...
	$uploadurl = urlencode(join_plugin_action2("swfupload").($iscamera ? '&op=screen' : ($isdoodle ? '&op=doodle&from=':'')));
	//TODO 修改权限
	...
	$feedurl = urlencode(join_plugin_action2("swfupload").'&op=finish&random='.random(8).'&albumid=');
	$albumurl = urlencode(join_plugin_action2("index").($isdoodle ? '&picid=' : '&id='));
	...
	if($uploadfiles && is_array($uploadfiles)) {
		...
		if($op == "doodle") {
			$fileurl = pic_get($uploadfiles['filepath'], 'plugin_groupalbums', $uploadfiles['thumb'], $uploadfiles['remote'], 0);
	...
	if($_FILES["Filedata"]['error']) {
		...
		$uploadfiles = pic_save_group($_FILES["Filedata"], $_POST['albumid'], addslashes(diconv(urldecode($_POST['title']), 'UTF-8')));
	}
	$proid = $_POST['proid'];
	$uploadResponse = true;
	$albumid = 0;
...
include template("groupalbum2:swfupload");
