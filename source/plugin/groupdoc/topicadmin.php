...
$threadlist = $loglist = $has_tids = array();
...
if($tids = dimplode($_G['gp_moderate'])) {
	...
	$query = DB::query("SELECT * FROM ".DB::table("group_doc")." WHERE docid IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0'");
	while($thread = DB::fetch($query)) {
		$has_tids[] = $thread['docid'];
	}
	
	foreach($_G['gp_moderate'] as $tid){
		if(empty($has_tids) || !in_array($tid, $has_tids)){
			DB::insert('group_doc', array('docid' => $tid, 'fid' => $_G['fid']));
		}
	}
	
	$query = DB::query("SELECT * FROM ".DB::table("group_doc")." WHERE docid IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0'");
	while($thread = DB::fetch($query)) {
		$thread['id'] = $thread['docid'];
...
switch($frommodcp) {
	case '1':
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=groupdoc&plugin_op=groupmenu";
		break;
	case '2':
...
if(!submitcheck('modsubmit')) {
	...
	if($_G['gp_optgroup'] == 1 && $single) {
		...
		$typeselect = grouptypeselect(0, 'groupdoc');
	...
	include template('groupdoc:topicadmin');
	exit();
} else {
	$moderate_ids = array_keys($threadlist);
...
//			}
			...
			if($operation == 'stick') {
				...
				$query2 = DB::query("SELECT * FROM ".DB::table('group_doc')." WHERE docid IN ($moderatetids)");
				while($value = DB::fetch($query2)){
					...
					if($displayorder){
						foreach($moderate_ids as $id){
							feed_notify_stick_doc($id);
						}
					}else{
						foreach($moderate_ids as $id){
							feed_notify_stick_no_doc($id);
						}				
					}
				}
				
				DB::query("UPDATE ".DB::table('group_doc')." SET displayorder='$displayorder', moderated='1' WHERE docid IN ($moderatetids)");
				...
				DB::query("UPDATE ".DB::table('group_doc')." SET highlight='$highlight_style$highlight_color', moderated='1' WHERE docid IN ($moderatetids)", 'UNBUFFERED');
				...
				while($value = DB::fetch($query2)){
					...
					if($digestlevel){
						foreach($moderate_ids as $id){
							feed_notify_digest_doc($id);
						}
					}else{
						foreach($moderate_ids as $id){
							feed_notify_digest_no_doc($id);
						}
					}
				}
				
				DB::query("UPDATE ".DB::table('group_doc')." SET digest='$digestlevel', moderated='1' WHERE docid IN ($moderatetids)");
...
//				delgroupcache($_G["fid"], "digest");
				...
				$doclists = array();
				foreach($moderate_ids as $docid){
					$doclists[] = getFile($docid);	
				}
				
				deleteFiles($_G['uid'], $moderate_ids);
				
//				$str_uids = '';
//				foreach($doclists as $doc){
//					if($doc && !empty($doc['id']) && $doc['userid'] != $_G['uid']){
//						notification_add($doc['userid'], 'doc', 'doc_delete', array('group' => "<a href=\"forum.php?mod=group&fid=$_G[fid]\" target=\"_blank\">".$_G['forum']['name']."</a>", 'actor' => "<a href=\"home.php?mod=space&uid=$doc[userid]\" target=\"_blank\">$doc[username]</a>", 'title' => $doc['title']), 1);
//					}
//				}
				...
				changeFolder($_G['uid'], $_G['gp_typeid'], $moderate_ids);

				foreach($doclists as $doc){
					if($doc && !empty($doc['id']) && $doc['zoneid'] != $_G['gp_typeid'] && $doc['userid'] != $_G['uid']){
						feed_notify_type_doc($doc, $categorys, $categorys[$_G['gp_typeid']]);
					}
				}
...
//精华动态 & 通知
function feed_notify_digest_doc($id){
	global $_G;
	...
	if($doc && !empty($doc['id'])){
		...
		$feed['idtype'] = 'gdocid';
		$feed['uid'] = $doc['userid'];
		$feed['username'] = $doc['username'];
		$feed['dateline'] = $doc['uploadtime'];
		
		$url = $doc['titlelink'];
		
		$feed['title_template'] = 'feed_digest_doc_title';
		$feed['body_template'] = 'feed_digest_doc_body';
		$feed['body_data'] = array(
			'title' => "<a href=\"$url\" target=\"_blank\">$doc[title]</a>",
		);
		$feed = mkfeed($feed);
		...
		if($doc['userid'] != $_G['uid']){
			$note_type = 'doc';
			$note = 'gdoc_digest';
			$note_values = array('group' => "<a href=\"forum.php?mod=group&fid=$doc[zoneid]\" target=\"_blank\">".$_G['forum']['name']."</a>", 'title'=>"$doc[title]", 'actor' => "<a href=\"home.php?mod=space&uid=$doc[userid]\" target=\"_blank\">$doc[username]</a>");
		...
		//经验值
		require_once libfile('function/group');
		group_add_empirical_by_setting($doc['userid'], $doc['zoneid'], 'topic_competitive', $doc[id]);
	
		//积分
		require_once libfile('function/credit');
		credit_create_credit_log($doc['userid'], 'digest', $doc['id']);	
	}
}

//撤销精华动态 & 通知
function feed_notify_digest_no_doc($id){
	global $_G;
	...
	if($doc && !empty($doc['id'])){
		...
		$feed['title_template'] = 'feed_digest_no_doc_title';
		$feed['body_template'] = 'feed_digest_no_doc_body';
		$feed['body_data'] = array(
			'title' => "<a href=\"$url\" target=\"_blank\">$doc[title]</a>",
		);
		$feed = mkfeed($feed);
		...
		if($doc['userid'] != $_G['uid']){
			$note_type = 'doc';
			$note = 'gdoc_digest_no';
			$note_values = array('group' => "<a href=\"forum.php?mod=group&fid=$_G[fid]\" target=\"_blank\">".$_G['forum']['name']."</a>", 'title'=>"$doc[title]", 'actor' => "<a href=\"home.php?mod=space&uid=$doc[userid]\" target=\"_blank\">$doc[username]</a>");
...
//置顶动态 & 通知
function feed_notify_stick_doc($id){
	global $_G;
	...
	if($doc && !empty($doc['id'])){
		...
		$feed['title_template'] = 'feed_stick_doc_title';
		$feed['body_template'] = 'feed_stick_doc_body';
		$feed['body_data'] = array(
			'title' => "<a href=\"$url\" target=\"_blank\">$doc[title]</a>",
		);
		$feed = mkfeed($feed);
		...
		//经验值
		require_once libfile('function/group');
		group_add_empirical_by_setting($doc['userid'], $doc['zoneid'], 'topic_top', $doc[id]);
	
		//积分
		require_once libfile('function/credit');
		credit_create_credit_log($doc['userid'], 'top', $doc['id']);	
	}
}

function feed_notify_stick_no_doc($id){
	global $_G;
	...
	if($doc && !empty($doc['id'])){
		...
		$feed['title_template'] = 'feed_stick_no_doc_title';
		$feed['body_template'] = 'feed_stick_no_doc_body';
		$feed['body_data'] = array(
			'title' => "<a href=\"$url\" target=\"_blank\">$doc[title]</a>",
		);
		$feed = mkfeed($feed);
...
//分类移动 动态& 通知
function feed_notify_type_doc($doc, $type, $type2){
	global $_G;
	...
	if($doc && !empty($doc['id'])){
		if($doc['userid'] != $_G['uid']){
			$note_type = 'doc';
			$note = 'gdoc_type';
			$note_values = array('group' => "<a href=\"forum.php?mod=group&fid=$_G[fid]\" target=\"_blank\">".$_G['forum']['name']."</a>", 'title'=>"$doc[title]", 'actor' => "<a href=\"home.php?mod=space&uid=$doc[userid]\" target=\"_blank\">$doc[username]</a>", 'type1' => $type[$doc['folderid']]['name'], 'type2' => $type2['name']);
