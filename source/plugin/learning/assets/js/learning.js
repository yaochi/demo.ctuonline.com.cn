...
function searchFocusL2(obj) {
if(obj.value == '请输入姓名') {
obj.value = '';
}

}
function searchBlurL2(obj) {
if(obj.value == '' ) {
obj.value = '请输入姓名';
...
jQuery(function(){
	jQuery('.exampleBtn').click(function(){
		var flag = jQuery(this).hasClass('exampleOff');
		if (flag) {
			jQuery(this).removeClass('exampleOff');
			jQuery(this).html('隐藏样例');
			jQuery(this).nextAll('.example').slideDown(200);
		}else{
			jQuery(this).addClass('exampleOff');
			jQuery(this).html('展开样例');
			jQuery(this).nextAll('.example').slideUp(200);
		}
	});
