...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("forum_forum_lecturer")." WHERE id IN ($tids) LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=lecturer&plugin_op=groupmenu";
		break;
}
...
if(!submitcheck('modsubmit')) {
	...
	include template('forum/lecturertopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'delete') {
				...
				DB::query("DELETE FROM ".DB::table('forum_forum_lecturer')." WHERE id IN ($moderatetids)"." AND fid=".$_G['fid']);
