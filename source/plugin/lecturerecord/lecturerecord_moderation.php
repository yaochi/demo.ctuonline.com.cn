...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("lecture_record")." WHERE id IN ($tids) LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=lecturerecord&plugin_op=groupmenu";
		break;
}
...
if(!submitcheck('modsubmit')) {
	...
	include template('forum/lecturerecordtopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'delete') {
				...
				$query = DB::query("SELECT lr.*, l.lecid lecid FROM ".DB::table("lecture_record")." lr, ".DB::table("lecturer")." l WHERE lr.teacher_id=l.id AND lr.id IN ($tids) LIMIT $_G[tpp]");
				while($thread = DB::fetch($query)) {
					$lecrecords[] = $thread;
				}
				DB::query("DELETE FROM ".DB::table('lecture_record')." WHERE id IN ($moderatetids)");
				
				
				require_once libfile('function/core');
				require_once libfile('function/credit');
				$resultids = explode(',',$moderatetids);
				foreach ($lecrecords as $lecrecord) {
					$userinfo = getlecturerbyid($lecrecord[teacher_id]);
					//经验值
					$empirical = getoldempirical($lecrecord);
					if ($empirical>0) {
						group_add_empirical($userinfo[lecid], $_G['fid'], (0-$empirical));
					}
					//通知
					notification_add($lecrecord[lecid],'zq_teachlog','[授课记录]您的“{title}”的授课记录被“{actor}”删除了，快去看看吧。',array("title"=>$lecrecord[name]), 0);
					//积分
					if ($lecrecord["type"]==1) {
						$creditaction = "deletecourseapply";
					} elseif ($lecrecord["type"]==2) {
						$creditaction = "deletecousedevelop";
					} elseif ($lecrecord["type"]==3) {
						$creditaction = "deletecourseresearch";
					} elseif ($lecrecord["type"]==4) {
						$creditaction = "deletecourseguide";
					}
					credit_create_credit_log($lecrecord[lecid], $creditaction, $lecrecord["id"]);
				}
