...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("notice")." WHERE id IN ($tids) AND group_id='$_G[fid]' AND displayorder>='0' AND digest>='0' LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=notice&plugin_op=groupmenu";
		break;
}
...
//	分类
	...
	include template('forum/noticetopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			//	积分
				...
				DB::query("UPDATE ".DB::table('notice')." SET moderated='-1' WHERE id IN ($moderatetids)");
				...
				DB::query("UPDATE ".DB::table('notice')." SET status=".$_POST["pstatus"]." WHERE id IN ($moderatetids)");
