 ...
 */
class block_delete_poll{
	function deletedata($ids,$contenttype,$fid){
	 	if($ids){
			foreach($ids as $id){
				DB::query("DELETE FROM ".DB::table('forum_poll')." WHERE tid='$id'");
				DB::query("DELETE FROM ".DB::table('forum_polloption')." WHERE tid='$id'");
				DB::query("DELETE FROM ".DB::table('forum_pollvoter')." WHERE tid='$id'");
