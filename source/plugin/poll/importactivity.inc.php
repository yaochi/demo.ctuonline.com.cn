...
function index() {
    ...
    if ($result) {
		...
		WHERE t.fid=$fup  AND special=1 AND t.displayorder IN (0, 1, 2, 3, 4)";
        $query = DB::query($sql);
...
function create() {
	...
	//$pollsql= "SELECT p.* FROM ".DB::table("forum_poll")." p WHERE tid IN(" . implode(",", $ids) . ")";
    $query = DB::query($sql);
	//$pollquery = DB::query($pollsql);
    $tids = array();
    $count = 0;
    while ($row = DB::fetch($query)) {
        $count++;
        ...
        DB::query($sql);
        $tid = DB::insert_id();
		hook_create_resource($tid, "poll",$fid);
		$pollsql= "SELECT p.* FROM ".DB::table("forum_poll")." p WHERE tid = $row[tid]";
		$pollquery = DB::query($pollsql);
		while($pollrow=DB::fetch($pollquery)){
			$pollsql= "INSERT INTO " . DB::table('forum_poll') . " (tid,overt, multiple, visible, maxchoices, expiration, pollpreview, voters)
		VALUES ($tid,'$pollrow[overt]', '$pollrow[multiple]', '$pollrow[visible]','$pollrow[maxchoices]','0', '$pollrow[pollpreview]','0')";
			 DB::query($pollsql);
		 	$polloptionsql= "SELECT p.* FROM ".DB::table("forum_polloption")." p WHERE tid = $row[tid]";
		 	$polloptionquery = DB::query($polloptionsql);
			while($polloptionrow=DB::fetch($polloptionquery)){
				$polloptionsql= "INSERT INTO " . DB::table('forum_polloption') . " (tid,votes, displayorder,polloption )
		VALUES ($tid,'0', '$polloptionrow[displayorder]','$polloptionrow[polloption]')";
			 DB::query($polloptionsql);
			}
