...
class block_notsolvedquestion{

	function getsetting() {
		$plugin_id = array_pop(explode(DIRECTORY_SEPARATOR, dirname(dirname(__FILE__))));//$plugin_id值为当前文件所在目录的倒数第二级目录名   ,这里$plugin_id的值为groupmember
        return array(
            array(
            	"type"=>'<input type="hidden" name="parameter[plugin_id]" value="'.$plugin_id.'"/>'
            )
        );
	}
	function getstylesetting($style) {
  	...
  	function getdata($style, $parameter) {
  		...
  		$query=DB::query("SELECT ft.*, cmp.realname realname FROM ".DB::table('forum_thread')." ft, ".DB::table("common_member_profile")." cmp WHERE ft.authorid=cmp.uid AND ft.fid='$fid' AND ft.special=3 AND ft.question_status='N' AND ft.displayorder!='-1'  ORDER BY ft.dateline desc limit $shownum");
		...
		$isEnableAndprefix=common_category_isEnableAndprefix($fid, 'qbar');
  		while($question=DB::fetch($query)){
			...
			if ($style[key] == "general"&&$parameter["title_len"]) {
				...
				$question['subject']=cutstr($question['subject'], $parameter["title_len"]);
				$question['name']=$question['subject'];
			}elseif($style[key] == "question_multi3"&&$parameter["title_len"]){
				$question['name']=cutstr($question['subject'], $parameter["title_len"]);
			}elseif($style[key] == "question_multi2"&&$parameter["title_len"]){
				$question['name']=cutstr($question['subject'], $parameter["title_len"]);
			}else{
				$question['name']=$question['subject'];
			}		
		...
		$result=array();
		$result["parameter"] = $parameter;//add by qiaoyz,2011-2-25
