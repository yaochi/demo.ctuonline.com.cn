...
class block_solvedquestion{
	...
	function getstylesetting($style) {
  	...
  	function getdata($style, $parameter) {
  		...
  		$query=DB::query("SELECT ft.*, cmp.realname realname FROM ".DB::table('forum_thread')." ft, ".DB::table("common_member_profile")." cmp WHERE ft.authorid=cmp.uid AND ft.fid='$fid' AND ft.special=3 AND ft.question_status='Y' AND ft.displayorder!='-1' ORDER BY ft.dateline desc limit $shownum");
