...
$discuz = & discuz_core::instance();//初始化discuz_core对象并返回一个句柄
...
//exit("你好，中国！index.php");
$mod = !in_array($discuz->var['mod'], $modarray) ? 'index' : $discuz->var['mod'];//$discuz->var是& $_G
...
loadforum();//函数位于function_forum.php文件中，完成的功能有将专区的一些信息存放到$_G中去
set_rssauth();
runhooks();//?


/*
 * 以下代码源自forum/source/module/forum/forum_forumdisplay.php文件
 */
...
$_G['action']['fid'] = $_G['fid'];//$_G['fid']在49行调用loadforum函数时被设置为http请求参数fid的值

$_G['gp_specialtype'] = isset($_G['gp_specialtype']) ? $_G['gp_specialtype'] : '';//在帖子表中special指的事帖子类型：0：话题，1：投票，3：悬赏 ，5：辩论
$_G['gp_dateline'] = isset($_G['gp_dateline']) ? intval($_G['gp_dateline']) : 0;//时间
$_G['gp_digest'] = isset($_G['gp_digest']) ? $_G['gp_digest'] : '';//是否精华
...
$_G['forum']['extra'] = unserialize($_G['forum']['extra']);//$_G['forum']['extra']存放什么信息？
...
$forumarchive = array();//archive存档，档案文件
if($_G['forum']['archive']) {
...
if($_G['forum']['status'] == 3) {//$_G['forum']['status']记录什么信息？
...
$view=$_GET['view'];
$filteradd .= "AND special='3'";//fumz,2010-7-23 14:28:52，过滤帖子（悬赏）
if($view=='solved'){
	$filteradd.="AND question_status='Y' ";
}else if($view=='notsolved'){
	$filteradd.="AND question_status='N' ";
}else{
	$view="all";
}
$category_id=$_GET['category_id'];//fumz,2010-8-5 13:23:29,按分类浏览提问
if($category_id){
	$filteradd.="AND category_id=$category_id ";
}
$keyword=$_REQUEST['keyword'];//提问搜索的关键字信息 
if($keyword){
	$filteradd.="AND subject like '%".$keyword."%' ";
}
$plugin_name=$_GET['plugin_name'];//fumz,2010-8-5 17:47:45


require_once libfile("function/category");
$pluginid=$_G["gp_plugin_name"];
...
//annotated by fumz,2010-8-9 15:42:16;解决分页不正确的问题
//if(empty($_G['gp_filter']) && empty($_G['gp_sortid']) && empty($_G['gp_archiveid']) && empty($_G['forum']['archive'])&&$view!='solved'&&$view!='notsolved'&&$category_id==0) {//modified by fumz,2010-8-5 10:50:40
//	$_G['forum_threadcount'] = $_G['forum']['threads'];
//	
//} else {
//	$_G['forum_threadcount'] = DB::result_first("SELECT COUNT(*) FROM ".DB::table($threadtable)." WHERE fid='{$_G['fid']}' $filteradd AND displayorder>='0' AND special=3");
//	
//}
$_G['forum_threadcount'] = DB::result_first("SELECT COUNT(*) FROM ".DB::table($threadtable)." WHERE fid='{$_G['fid']}' $filteradd AND displayorder>='0' AND special=3");
...
$multipage = multi($_G['forum_threadcount'], 
					$_G['tpp'],
					 $page, 
					 "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=qbar&view=$view&category_id=$category_id&keyword=$keyword&plugin_op=groupmenu".($_G['gp_filter'] ? "&filter=$_G[gp_filter]" : '')."$forumdisplayadd[orderby]$forumdisplayadd[page]$multipage_archive", 
					 $_G['setting']['threadmaxpages']);
...
//begin
require_once libfile("function/category");
$qis_enable_category = common_category_is_enable($_G["fid"], 'qbar');//fumz,提问吧分类是否开启
$qisprefix=common_category_is_prefix($_G['fid'], 'qbar');//fumz,提问是否显示分类
//end
while(($querysticky && $thread = DB::fetch($querysticky)) || ($query && $thread = DB::fetch($query))) {
	...
	/**
	 * added by fumz,2010-8-18 14:14:21
	 * 添加分类信息
	 */
	//begin
//	require_once libfile("function/category");
...
//	$qisprefix=common_category_is_prefix($_G['fid'], 'qbar');//提问是否显示分类	
	if($qis_enable_category&&$qisprefix){
		if($thread['category_id']!=0){
			$catesql="SELECT name from pre_common_category WHERE id=".$thread['category_id'];
			$catequery=DB::query($catesql);
			$catename=DB::fetch($catequery,0);
			if(empty($catename)){
				$thread['category_name']="未分类";
			}else{
				$thread['category_name']=$catename['name'];		
			}			
		}else{
			$thread['category_name']="未分类";
		}		
...
if(!empty($grouptids)) {
	$query = DB::query("SELECT t.tid, f.name, f.fid FROM ".DB::table('forum_thread')." t LEFT JOIN ".DB::table('forum_forum')." f ON f.fid=t.fid WHERE t.tid IN(".dimplode($grouptids).")");//添加检索条件 AND t.special='3'
