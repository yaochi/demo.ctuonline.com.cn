...
/*
 * copied from source/module/forum/forum_post.php
 * 
 * 
*/
define('NOROBOT', TRUE);
...
//if(empty($_G['gp_action'])) {//提问吧插件的$_G['gp_action']的值为plugin
...
if($_G['gp_action'] == 'reply') {
	...
	$addfeedcheck = !empty($space['privacy']['feed']['newthread']) ? 'checked="checked"': '';//检查用户发表提问的权限
}
...
if($_G['forum']['status'] == 3) {//$_G['forum']['status'] == 3代表什么？
...
require_once libfile("function/group");//引入/source/function/function_group.php文件
//$uid=$_G['uid'];
//$fid=$_G['fid'];
//exit("你好$fid");
$totalEmpiricalValue=group_get_empirical($_G['uid'],$_G['fid']);//用户在当前专区的经验值（该函数在function_group.php文件中）
