...
function modnotices(optgroup, operation) {
...
function tmodclick(obj) {
	...
	if(modclickcount > 0) {
		...
		while((obj = obj.offsetParent).id != 'questionarylist') {
...
function tmodnotices(optgroup, operation) {
	var checked = 0;
	var operation = !operation ? '' : operation;
	...
	if(!checked) {
		...
		showNoticeWindow('mods', 'moderate', 'post');
	}
}

function showNoticeWindow(k, url, mode, cache, menuv) {
