...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("questionary")." WHERE questid IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0' LIMIT $_G[tpp]");
...
//		$thread['lastpost'] = dgmdate($thread['lastpost'], 'u');
		$threadlist[$thread['questid']] = $thread;
		$_G['tid'] = empty($_G['tid']) ? $thread['questid'] : $_G['tid'];
	}
}
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=questionary&plugin_op=groupmenu";
		break;
}
...
//	分类
	...
	include template('forum/questionarytopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				$opquerys=DB::query("SELECT * FROM ".DB::table("questionary")." WHERE questid IN ($moderatetids)");
				while($opquery = DB::fetch($opquerys)) {
				if($displayorder==$opquery[displayorder]){
				}
				else if($displayorder=='0'){
					notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的资源被['.user_get_user_name_by_username($_G['member']['username']).']撤销置顶了，赶快去看看吧', array(), 1);
					}else{
					notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的资源被['.user_get_user_name_by_username($_G['member']['username']).']置顶了，赶快去看看吧', array(), 1);
					}
				}
				DB::query("UPDATE ".DB::table('questionary')." SET displayorder='$displayorder', moderated='1' WHERE questid IN ($moderatetids)");
				...
				DB::query("UPDATE ".DB::table('questionary')." SET highlight='$highlight_style$highlight_color', moderated='1' WHERE questid IN ($moderatetids)", 'UNBUFFERED');
				...
				while($opquery = DB::fetch($opquerys)) {
					if($digestlevel==$opquery[digest]){
						...
						notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的资源被['.user_get_user_name_by_username($_G['member']['username']).']撤销加精了，赶快去看看吧', array(), 1);
						}else{
						notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的资源被['.user_get_user_name_by_username($_G['member']['username']).']加精了，赶快去看看吧', array(), 1);
						}
				}
				DB::query("UPDATE ".DB::table('questionary')." SET digest='$digestlevel', moderated='1' WHERE questid IN ($moderatetids)");
				...
				//通知
				foreach($threadlist as $thread){
					hook_delete_resource($thread[questid],'question');
				}
				require_once libfile('function/core');
				$opquerys=DB::query("SELECT * FROM ".DB::table("questionary")." WHERE questid IN ($moderatetids)");
				while($opquery = DB::fetch($opquerys)) {
					notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的资源被['.user_get_user_name_by_username($_G['member']['username']).']删除了', array(), 1);
				}
				DB::query("UPDATE ".DB::table('questionary')." SET moderated='-1' WHERE questid IN ($moderatetids)");
				...
				notification_add($opquery[uid],"zq_question",'[问卷]您“'.$_G[forum][name].'”的专区的“'.$opquery[questname].'”的问卷被['.user_get_user_name_by_username($_G['member']['username']).']从“'.$oldcategory[name].'”分类下移动到“'.$newcategory[name].'"分类下，赶快去看看吧', array(), 1);
				}
				DB::query("UPDATE ".DB::table('questionary')." SET classid=".$_POST["category"]." WHERE questid IN ($moderatetids)");
