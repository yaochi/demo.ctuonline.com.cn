...
function modresources(optgroup, operation) {
...
function tmodclick(obj) {
	...
	if(modclickcount > 0) {
		...
		while((obj = obj.offsetParent).id != 'resourcelist') {
...
function tmodresources(optgroup, operation) {
	var checked = 0;
	var operation = !operation ? '' : operation;
	...
	if(!checked) {
		...
		showResourceWindow('mods', 'moderate', 'post');
	}
}

function showResourceWindow(k, url, mode, cache, menuv) {
