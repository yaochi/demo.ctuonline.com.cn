...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("resourcelist")." WHERE id IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0' LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=resourcelist&plugin_op=groupmenu";
		break;
}
...
//	分类
	...
	include template('forum/resourcelisttopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				DB::query("UPDATE ".DB::table('resourcelist')." SET displayorder='$displayorder' WHERE id IN ($moderatetids)");
			...
			//通知
				...
				DB::query("UPDATE ".DB::table('resourcelist')." SET highlight='$highlight_style$highlight_color' WHERE id IN ($moderatetids)", 'UNBUFFERED');
				...
				DB::query("UPDATE ".DB::table('resourcelist')." SET digest='$digestlevel' WHERE id IN ($moderatetids)");
				...
				DB::query("DELETE FROM ".DB::table('resourcelist')." WHERE id IN ($moderatetids)");
			...
			//通知
				...
				DB::query("UPDATE ".DB::table('resourcelist')." SET fcategoryid=".$_POST["category"]." WHERE id IN ($moderatetids)");
