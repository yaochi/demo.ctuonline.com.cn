...
class block_shlecturer {
    ...
    function getsetting() {
        ...
        $query = DB::query("SELECT * FROM ".DB::table('shlecture'));
    ...
    function getdata($style, $parameter) {
        ...
        if ($lecid) {
	        $query = DB::query("SELECT * FROM ".DB::table('shlecture')." where id=".$lecid);
	        
	        $lecturers = array();
	    	if ($style[key] == "selfstyle") {
	            //图片尺寸  摘要长度  标题字数
	            while ($lecturer = DB::fetch($query)) {
	                $lecturer["url"] = "forum.php?mod=group&action=plugin&fid=".$_GET["fid"]."&plugin_name=shlecturer&plugin_op=viewmenu&diy=&lecid={$lecturer['id']}&lecturer_action=index";
					if(strlen($lecturer["bginfo"])>$parameter[top_desc_len]){
						$lecturer["bginfo"] = cutstr($lecturer["bginfo"], $parameter[top_desc_len]) . "...";
					}
