...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("shlecture")." WHERE id IN ($tids) LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=shlecturer&plugin_op=groupmenu";
		break;
}
...
if(!submitcheck('modsubmit')) {
	...
	include template('forum/shlecturertopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'delete') {
				...
				DB::query("DELETE FROM ".DB::table('shlecture')." WHERE id IN ($moderatetids)");
				DB::query("DELETE FROM ".DB::table('shlecture_direct')." WHERE lecid IN ($moderatetids)");
