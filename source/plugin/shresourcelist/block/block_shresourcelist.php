...
class block_shresourcelist {
    ...
    function getdata($style, $parameter) {
        ...
        $sql = "SELECT * FROM ".DB::table('shresourcelist')." WHERE ".$groupsql.$mytypesql.$mycategorysql." ORDER BY displayorder DESC, uploaddate DESC LIMIT ".$parameter['items'];
        $query = DB::query($sql);
        $resources = array();
		...
		if ($style[key] == "general") {
            ...
            while ($resource = DB::fetch($query)) {
                ...
                $resource["name"] = cutstr($resource["title"], $parameter["title_len"]);
                if ($resource["about"]) {
                    $resource["description"] = cutstr($resource["about"], $parameter[top_desc_len]);
                }
