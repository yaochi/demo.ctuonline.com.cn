...
//        }
		...
		$query = DB::query("SELECT * FROM ".DB::table('shresourcelist')." WHERE fid=".$groupid.$mycategory);
		while($value=DB::fetch($query)) {
			...
			$three .= '<option value="'.$value['id'].'"'.$check.' >'.$value['resourcealiss'].'</option>';
		}
        $three .='</select>';
...
//		$result[] = array("title"=>"选择类型", "html"=>$two);
    ...
    function getdata($style, $parameter) {
		...
		if ($resourceid) {
        $query = DB::query("SELECT * FROM ".DB::table('shresourcelist')." WHERE id IN (".$resourceid.") ORDER BY uploaddate DESC ");
        $resources = array();
