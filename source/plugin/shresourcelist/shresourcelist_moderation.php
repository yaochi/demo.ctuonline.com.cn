...
if($tids = dimplode($_G['gp_moderate'])) {
	$query = DB::query("SELECT * FROM ".DB::table("shresourcelist")." WHERE id IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0' LIMIT $_G[tpp]");
...
switch($frommodcp) {
	...
	case '2':
		...
		break;
	default:
		$_G['referer'] = "forum.php?mod=group&action=plugin&fid=$_G[fid]&plugin_name=shresourcelist&plugin_op=groupmenu";
		break;
}
...
//	分类
	...
	include template('forum/shresourcelisttopicadmin');
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				DB::query("UPDATE ".DB::table('shresourcelist')." SET displayorder='$displayorder' WHERE id IN ($moderatetids)");
				
			//通知
				require_once libfile('function/core');
				if ($displayorder==0) {
					$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}撤销置顶了，赶快去看看吧';
				} elseif ($displayorder==1) {
					$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}置顶了，赶快去看看吧';
				}
				foreach ($threadlist as $thread) {
					if ($thread[displayorder] != $displayorder) {
						$groupname = get_groupname_by_fid($thread[fid]);
						notification_add($thread[uid],'zq_resources',$desc,array("groupname"=>'<a href="forum.php?mod=group&fid='.$thread[fid].'">'.$groupname.'</a>', "title"=>$thread[title]), 0);
					}
				}
				
			} elseif($operation == 'highlight') {
				if(!$_G['group']['allowhighlightthread']) {
					showmessage('undefined_action22');
				}
				...
				DB::query("UPDATE ".DB::table('shresourcelist')." SET highlight='$highlight_style$highlight_color' WHERE id IN ($moderatetids)", 'UNBUFFERED');
				...
				DB::query("UPDATE ".DB::table('shresourcelist')." SET digest='$digestlevel' WHERE id IN ($moderatetids)");
				
				//通知
				require_once libfile('function/core');
				if ($digestlevel==0) {
					$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}撤销加精了，赶快去看看吧';
				} elseif ($digestlevel==1) {
					$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}加精了，赶快去看看吧';
				}
				foreach ($threadlist as $thread) {
				...
				DB::query("DELETE FROM ".DB::table('shresourcelist')." WHERE id IN ($moderatetids)");
				
			//通知
				require_once libfile('function/core');
				$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}删除了，赶快去看看吧';
				foreach ($threadlist as $thread) {
					$groupname = get_groupname_by_fid($thread[fid]);
					notification_add($thread[uid],'zq_resources',$desc,array("groupname"=>'<a href="forum.php?mod=group&fid='.$thread[fid].'">'.$groupname.'</a>', "title"=>$thread[title]), 0);
				}
				...
				DB::query("UPDATE ".DB::table('shresourcelist')." SET fcategoryid=".$_POST["category"]." WHERE id IN ($moderatetids)");
				
				//通知
				require_once libfile('function/core');
				require_once libfile('function/category');
				$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}从“{catefrom}”分类下移动到“{cateto}"分类下，赶快去看看吧';
				foreach ($threadlist as $thread) {
					//获取分类名称
					if (!$thread[fcategoryid]) {
						$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}移动到“{cateto}"分类下，赶快去看看吧';
						if (!$_POST["category"]) {
							$desc = '[资源]您“{groupname}”的专区的“{title}”的资源被{actor}取消了分类，赶快去看看吧';
						}
					} else {
						$catefrom = common_category_get_category_by_id($thread[fcategoryid]);
					}
					$cateto = common_category_get_category_by_id($_POST["category"]);
					$groupname = get_groupname_by_fid($thread[fid]);
					notification_add($thread[uid],'zq_resources',$desc,
						array("groupname"=>'<a href="forum.php?mod=group&fid='.$thread[fid].'">'.$groupname.'</a>', 
							"title"=>$thread[title], 
							"catefrom"=>$catefrom[name], 
							"cateto"=>$cateto[name]
						), 0);
				}
