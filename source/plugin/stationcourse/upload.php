<?php
	...
	if($type==1) {
		...
		$upload="course";
		$new_file_name="course".$_G[fid].".xls";//上传过后的保存的名称
   		$actioname="importcourse";
	}
	else if($type==3) {
		$upload="station_course";
		$new_file_name="station_course".$_G[fid].".xls";//上传过后的保存的名称
   		$actioname="importstacourse";
   		$sid=$_POST['station_input_id'];
   		$sname=$_POST['station_input'];
   		$actioname.="&sid=".$sid."&sname=".$sname;
	}
     		...
     		//检查文件大小
  			...
  			switch($error)
    			...
    			case 0:
    				...
    				showmessage($info."上传成功！正在导入，请稍候！",$url);
