...
function proLectLevelsRenUC(event) {
	...
	var levelid=$("prolv" + currentSelLv).options[$("prolv" + currentSelLv).selectedIndex].value;
	if($("levelname") != null) {
		$("levelname").value = $("prolv" + currentSelLv).options[$("prolv" + currentSelLv).selectedIndex].text;
	}
	showWindow('proLectLv', 'forum.php?mod=group&action=plugin&fid=197&plugin_name=lecturermanage&plugin_op=createmenu&lecturermanage_action=operate&oper=changelevel&id='+levelid);
}
