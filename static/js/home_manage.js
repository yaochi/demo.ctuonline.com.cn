...
function docomment_colse(doid, key) {
	...
	var content=document.getElementById(showid);
	var num=content.getElementsByTagName("li").length;
	if(num>0) $(opid).innerHTML = '已有'+num+'条回复';
	else $(opid).innerHTML = '回复';
...
function getrealgroup(gid) {
	if(gid) {
		var x = new Ajax();
		x.get('home.php?mod=spacecp&ac=privacy&inajax=1&op=getrealgroup&gid='+gid, function(s){
			s = s + ' ';
			$('target_realnames').innerHTML += s;
		});
	}
}