...
function _formatUrl(url, mode, host, pathname) {
	...
	if ((match = /^(\w+:\/\/[^\/]*)/.exec(url))) {
		if (match[1] !== host) {
			if(url.indexOf("/data/kindeditorattached")!=-1){
			}else if(url.indexOf("/data/attachment/flv")!=-1){
				url="static/image/common/flvplayer.swf?&autostart=true&file="+url;
			}
			return url;
		}
	} else if (/^\w+:/.test(url)) {
		...
		function movePastedData() {
			...
			if (_WEBKIT) {
				...
				K('span[style]', div).each(function(){
				        K(this).css('white-space') == 'nowrap' && K(this).remove(true);
				});
			}
