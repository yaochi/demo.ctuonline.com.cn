function block_get_setting(classname, script, fid) {
	var x = new Ajax();
	x.get('portal.php?mod=portalcp&ac=block&op=setting&classname='+classname+'&script='+script+'&inajax=1'+"&fid="+fid, function(s){
		ajaxinnerhtml($('tbody_setting'), evalscript(s));
	});
}

function block_get_setting_array(classname, script, fid, param) {
	var x = new Ajax();
	var pa = '&select_type=' + param;
//	$(param).each(function(){ 
//		pa += '&' + this.key + '=' + this.value; 
//	});
	x.get('portal.php?mod=portalcp&ac=block&op=setting&classname='+classname+'&script='+script+'&inajax=1'+"&fid="+fid+pa, function(s){
		ajaxinnerhtml($('tbody_setting'), s);
	});
}

function block_get_setting_resourcelist_array(classname, script, fid, param) {
	var x = new Ajax();
	var pa = '&select_category=' + param;
//	$(param).each(function(){ 
//		pa += '&' + this.key + '=' + this.value; 
//	});
	x.get('portal.php?mod=portalcp&ac=block&op=setting&classname='+classname+'&script='+script+'&inajax=1'+"&fid="+fid+pa, function(s){
...
function block_get_setting_pic_array(classname, script, fid, param) {
	var x = new Ajax();
	var pa = '&select_album=' + param;
//	$(param).each(function(){ 
//		pa += '&' + this.key + '=' + this.value; 
//	});
	x.get('portal.php?mod=portalcp&ac=block&op=setting&classname='+classname+'&script='+script+'&inajax=1'+"&fid="+fid+pa, function(s){
...
function block_show_thumbsetting(classname, styleid, bid) {
	...
	x.get('portal.php?mod=portalcp&ac=block&op=thumbsetting&classname='+classname+"&script="+script+'&styleid='+styleid+'&bid='+bid+'&inajax=1&fid='+getArgs()["fid"], function(s){
...
function searchblock(from) {
	...
	reloadselection('portal.php?mod=portalcp&ac=portalblock&searchkey='+value+'&from='+from);
}
