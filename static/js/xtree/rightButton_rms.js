...
function DrawMouseMenu(){
	...
	var SysMenu=["document.execCommand('SelectAll')\">&nbsp;&nbsp;全选","MouseMenu.style.visibility='hidden';document.execCommand('SaveAs','true')\">&nbsp;&nbsp;另存为...","window.print()\">&nbsp;&nbsp;打印","window.location.reload()\">&nbsp;&nbsp;刷新"];
	var MenuStr="";
 ...
 * @param id
 * @param isleaf
 * @param isGroup	当前用户是否为集团管理员
 * @param isMyNode	是否是当前用户所在公司的节点
 * @param isDefault	是否为代节点
 * @param canAdd	是否可以增加子节点
 * @param canMod	是否可以修改当前节点
 * @param canDelete	是否可以删除当前节点
 * @param 
 * @param 
 */
function rightButton(id, isleaf, isGroup, isMyNode, canAdd, canMod, canDelete) {   
	MouseMenu.innerHTML = "";
	DivH=2;
	var HrStr="<tr><td align=center valign=middle height=2><TABLE border=0 cellpadding=0 cellspacing=0 width=128 height=2><tr><td height=1 bgcolor=buttonshadow><\/td><\/tr><tr><td height=1 bgcolor=buttonhighlight><\/td><\/tr><\/TABLE><\/td><\/tr>";
	var MenuStr1="<tr><td align=center valign=middle height=20><TABLE border=0 cellpadding=0 cellspacing=0 width=128><tr><td valign=middle height=16 width=100% class=Mout onMouseOver=this.className='Mover'; onMouseOut=this.className='Mout'; onclick=\"";
	...
	if (!isleaf) {
		//目录
			...
			XiciMenu=["\"><font color=\"#999999\">&nbsp;&nbsp;添加</font>","\"><font color=\"#999999\">&nbsp;&nbsp;修改</font>","\"> <font color=\"#999999\">&nbsp;&nbsp;删除</font>"];
	}else{
			XiciMenu=["_Add("+id+")\">&nbsp;&nbsp;添加","_Modify("+id+")\">&nbsp;&nbsp;修改","_Delete("+id+")\">&nbsp;&nbsp;删除"];	
	}
	*/
	XiciMenu=[canAdd || id == 0 ? "_Add("+id+")\">&nbsp;&nbsp;添加" :"\"> <font color=\"#999999\">&nbsp;&nbsp;添加</font>" ,canMod ? "_Modify("+id+")\">&nbsp;&nbsp;修改" : "\"> <font color=\"#999999\">&nbsp;&nbsp;修改</font>", canDelete ? "_Delete("+id+")\">&nbsp;&nbsp;删除" : "\"> <font color=\"#999999\">&nbsp;&nbsp;删除</font>"];
...
function _Add(id)
{
	//openwindowtocenter('../command/rmsControl?flag=rmsTypeAdd&parentid='+id+'&isgroup=1','CreatInfo',500,300);
	userFrame.window.location.replace("../command/rmsControl?flag=rmsTypeAdd&parentid="+id+"&isgroup=1");
}
function _Modify(id)
{
	//openwindowtocenter('../command/rmsControl?flag=rmsTypeModi&action&selId='+id+'&isgroup=1','openCategry',500,300);
	userFrame.window.location.replace("../command/rmsControl?flag=rmsTypeModi&selId="+id+"&isgroup=1");
}
...
function _Delete(id)
{
	...
	if (confirm("确定删除？")) 
	{
	    //openWindowbyDel('../command/rmsControl?flag=rmsTypeDel&id='+id+'&type=U','DeleteCategry',10,10);
	    userFrame.window.location.replace("../command/rmsControl?flag=rmsTypeDel&id="+id+"&type=U");
	}
}
