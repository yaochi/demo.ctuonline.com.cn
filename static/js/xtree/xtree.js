...
WebFXTreeAbstractNode.prototype.addparent = function (node, bNoIdent) {
	var pnode = this.parentNode;
	node.parentNode = pnode;
	pnode.childNodes[pnode.childNodes.length] = node;
	var root = pnode;
	if (pnode.childNodes.length >= 2) {
		pnode.childNodes[pnode.childNodes.length - 2]._last = false;
	}
	while (root.parentNode) { root = root.parentNode; }
	if (root.rendered) {
		if (pnode.childNodes.length >= 2) {
			document.getElementById(pnode.childNodes[pnode.childNodes.length - 2].id + '-plus').src = ((pnode.childNodes[pnode.childNodes.length -2].folder)?((pnode.childNodes[pnode.childNodes.length -2].open)?webFXTreeConfig.tMinusIcon:webFXTreeConfig.tPlusIcon):webFXTreeConfig.tIcon);
			pnode.childNodes[pnode.childNodes.length - 2].plusIcon = webFXTreeConfig.tPlusIcon;
			pnode.childNodes[pnode.childNodes.length - 2].minusIcon = webFXTreeConfig.tMinusIcon;
			pnode.childNodes[pnode.childNodes.length - 2]._last = false;
		}
		pnode._last = true;
		var foo = pnode;
		...
		webFXTreeHandler.insertHTMLBeforeEnd(document.getElementById(pnode.id + '-cont'), node.toString());
		if ((!pnode.folder) && (!pnode.openIcon)) {
			pnode.icon = webFXTreeConfig.folderIcon;
			pnode.openIcon = webFXTreeConfig.openFolderIcon;
		}
		if (!pnode.folder) { pnode.folder = true; pnode.collapse(true); }
		if (!bNoIdent) { pnode.indent(); }
	}
	return node;
}
...
//许文耀增加的2003－10－16
WebFXTreeAbstractNode.prototype.modify = function (strName) {
	var node = this;
	node.text = strName;
	var pNode = this.getPreviousSibling();
	...
	if(pNode != null){
		webFXTreeHandler.insertHTMLBeforeEnd(document.getElementById(pNode.id ), node.toString());
	}
	this.deSelect();
	node.select();
}
...
WebFXTreeItem.prototype.remove = function() {
	...
	this.parentNode.focus();//许文耀修改的2003－10－16
	this._remove();
